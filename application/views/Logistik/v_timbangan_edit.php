					...
					<div class="col-md-12">
						<div class="card card-info card-outline">
							<div class="card-header" style="padding:12px">
								...
								<input type="hidden" class="form-control" value="<?= $id_timb ?>" name="id_timb" id="id_timb">

								<input type="hidden" class="form-control" value="<?= $no_timb ?>" name="no_timb" id="no_timb">
							</div>
							...
							<div class="card-body row" style="padding-bottom:5px;font-weight:bold">						
								...
								<div class="col-md-3">
									...
									<input type="hidden" id="sts_input" value="edit">
									<input type="text" class="form-control" id="no_timbangan" name="no_timbangan" value="" readonly>
								</div>
							...
							<div class="card-body row" style="padding-bottom:5px;font-weight:bold">						
								...
								<div class="col-md-3">
									...
									<select id="plh_input" name="plh_input" name="plh_input" class="form-control select2" onchange="pilih_inp()">
							...
							<div class="card-body row" style="padding-bottom:5px;font-weight:bold;display:none" id="pil_kiriman" name="pil_kiriman" >		
...
<script type="text/javascript">
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data){
					...
					// 		set_po_edit('edit',index,val.id_item,data.header.id_pelanggan)
							
					// 		// var opt_produk = $("<option selected></option>").val(val.id_item).text(val.id_item);
					// 		// $('#item_po'+index).append(opt_produk).trigger('change');

					// 		list += `
					// 			<tr id="itemRow${no}">
					// 				<td id="detail-hapus-${no}">
					// 					<div class="text-center">
					// 						<a class="btn btn-danger" id="btn-hapus-${no}" onclick="removeRow(${no})"><i class="far fa-trash-alt" style="color:#fff"></i> </a>
					// 					</div>
					// 				</td>
					// 				<td>
					// 					<select class="form-control select2 narrow wrap wrap" name="item_po[${no}]" id="item_po${no}" style="width: 100%;" onchange="set_qty(this.value,${no})" selected>
					// 						<option value="">-- Pilih --</option>
					// 					</select>
					// 				</td>
					// 				<td>
					// 					<input type="text" name="qty_po[${no}]" id="qty_po${no}" class="angka form-control" value='${no}' onkeyup="ubah_angka(this.value,this.id)" readonly>
					// 				</td>
					// 				<td>
					// 					<input type="text" name="qty[${no}]" id="qty${no}" class="angka form-control" value='${val.berat_bahan}' onkeyup="ubah_angka(this.value,this.id),hitung_bb(),cek_bb(this.value,${no})" >	
					// 				</td>
					// 			</tr>
	...
	function set_po_edit(cek,id_item,val,pelanggan) 
	{
		$("#item_po"+id_item).val("").prop("readonly", false);
		...
		$.ajax({
			...
			success:function(data){			
				if(data.message == "Success"){						
					...
					if(index==id_item)
					{
						option += `<option selected value='${val.id}' detail=${val.bhn_bk}>
						[ ${val.kode_po} ]-[ ${val.nm_produk} ]-[ ${val.bhn_bk} ]</option>`;
					}else{
						option += `<option value='${val.id}' detail=${val.bhn_bk}>
						[ ${val.kode_po} ]-[ ${val.nm_produk} ]-[ ${val.bhn_bk} ]</option>`;
