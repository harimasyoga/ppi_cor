						...
						<div class="card-body row" style="padding:8px 0;font-weight:bold">
							<div class="col-md-2" style="padding-bottom:3px">
								<select class="form-control select2" id="rentang_thn" name="rentang_thn" onchange="load_data()">
									<?php
										$thang = date("Y");
										$qThWh = $this->db->query("SELECT YEAR(h.tgl_invoice) AS tahun FROM invoice_header h WHERE h.tgl_invoice LIKE '%$thang%' GROUP BY YEAR(h.tgl_invoice)");
										$qTahun = $this->db->query("SELECT YEAR(h.tgl_invoice) AS tahun FROM invoice_header h GROUP BY YEAR(h.tgl_invoice)");
										if($qThWh->num_rows() != 0){
...
<script type="text/javascript">
					...
					if (data.header.type == 'roll') {
						...
						list += `<tr>
							...
							<td style="text-align:center" colspan="2">
								<input type="text" id="pot_title" name="pot_title" class="form-control" value="" autocomplete="off" placeholder="POTONGAN">
							</td>
							<td style="text-align:center">
								<input type="text" id="pot_potongan" name="pot_potongan" class="form-control" style="text-align:right" onkeyup="ubah_angka(this.value,this.id)" value="" autocomplete="off" placeholder="0">
							</td>
							<td>
	...
	function addPotonganInv(i, opsi) {
		let id_inv = $("#id_inv").val()
		let no_inv_old = $("#no_inv_old").val()
		let pot_id = $("#pot_id"+i).val()
		let pot_title = $("#pot_title"+i).val()
		let pot_potongan = $("#pot_potongan"+i).val().split('.').join('')
		$.ajax({
			url: '<?php echo base_url('Logistik/addPotonganInv')?>',
			type: "POST",
			beforeSend: function() {
				swal({
					title: 'loading ...',
			...
			data: ({ id_inv, no_inv_old, pot_id, pot_title, pot_potongan, opsi }),
			success: function(res){
				data = JSON.parse(res)
				if(data.data){
					edit_data(id_inv, no_inv_old)
				}else{
					toastr.error(`<b>${data.msg}</b>`)
					swal.close()
				}
	...
	function updateInvMutasi() {
		$.ajax({
			url: '<?php echo base_url('Logistik/updateInvMutasi') ?>',
			type: "POST",
			beforeSend: function() {
				swal({
					title: 'Loading',
			...
			success: function(res) {
				data = JSON.parse(res)
				if (data.data) {
					toastr.success(`<b>UPDATE MUTASI BERHASIL!</b>`)
				} else {
