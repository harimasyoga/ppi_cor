				...
				<div class="col-md-7">
					<div id="card-produksi" style="display:none">
						...
						<div class="card card-success card-outline" style="padding-bottom:20px">
							...
							<?php if($this->session->userdata('level') == 'Admin' || $this->session->userdata('level') == 'PPIC' || $this->session->userdata('level') == 'Flexo' || $this->session->userdata('level') == 'User' || $this->session->userdata('level') == 'plan') { ?>
		...
		$.ajax({
			...
			success: function(res){
				...
				if(data.opsi != ''){
					...
					let htmlBtnGantiTgl = '';
					(data.plan_cor.total_prod_flexo == 0 && data.plan_cor.status_flexo == 'Open' && (urlAuth == 'Admin' || urlAuth == 'PPIC' || urlAuth == 'User' || urlAuth == 'plan')) ?
