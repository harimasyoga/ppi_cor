				...
				<div class="card-body row" style="padding:0 0 8px;font-weight:bold">
					<div class="col-md-2" style="padding-bottom:3px">
						<select id="tahun" class="form-control select2" onchange="load_data()">
							<?php 
								$thang = date("Y");
								$qThWh = $this->db->query("SELECT YEAR(p.eta_so) AS tahun FROM trs_so_detail p WHERE p.eta_so LIKE '%$thang%' GROUP BY YEAR(p.eta_so)");
								$qTahun = $this->db->query("SELECT YEAR(p.eta_so) AS tahun FROM trs_so_detail p GROUP BY YEAR(p.eta_so)");
								if($qThWh->num_rows() != 0){
