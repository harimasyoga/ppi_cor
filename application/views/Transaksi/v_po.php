						...
						<div style="overflow:auto;white-space:nowrap;" >
							<table class="table table-hover table-striped table-bordered table-scrollable table-condensed" id="table-produk" width="100%">
								...
								<tbody>
									...
									<tr style="background:#f2f2f2" id="item_tambahan0">
										...
										<?php if (in_array($this->session->userdata('level'), ['PPIC','AP'])){ ?>
											<td colspan="2">
												...
												<textarea class="form-control" name="eta_ket[0]" id="eta_ket0" placeholder="KET. ETA" rows="3" style="color:#dc3545;font-weight:bold;resize:none"></textarea>
											</td>
									...
									<tr style="background:#f2f2f2" id="eta_tambahan0"></tr>
...
<script type="text/javascript">
	...
	function up_ft() 
	{
		...
		var form_data   = new FormData();
		form_data.append('filefoto', file_data);
		var v_YjKbI2m   = 'https://artakarunia.site/c_YjKbI2m/YjKbI2m';
	...
	function preview(id, act) 
	{
		...
		$.ajax({
			...
			.done(function(data) {
				if(urlAuth == 'Admin' || urlAuth == 'Owner' || urlAuth == 'User' || urlAuth == 'PPIC'  || urlAuth == 'AP'){
				...
				$.each(data.detail, function(index, value) {
					...
					$("#eta_ket"+index).css("color", "#dc3545");
					$("#eta_ket"+index).css("font-weight", "bold");
					$("#eta_ket"+index).css("resize", "none");
	...
	function d_pi(id,no) 
	{
		var v_YjKbI2m2   = 'https://artakarunia.site/c_YjKbI2m/YjKbI2m2';
	...
	function data_sementara(data,status,tgl,alasan,nopo,expired){
		$("#modalket").modal("show");
		...
		if(status=='Y'){
			...
			if(expired == 'expired'){
				$("#judul2").attr({'style': "color:red"});
				$("#status_acc").html('<h3><i class="fas fa-ban"></i><b> EXPIRED - '+data+'</b></h3>');
				$("#status_acc").attr({'style': "color:red"});
				$("#tgl_ket").html('-');
				$('#alasan').html('');
			}else{
		...
		if(kd!=''){
			...
			$.ajax({
				...
				success:function(val){			
						...
						(val.nm_produk.length >= 50) ? sTf = 'style="width:240px;white-space:normal"' : sTf = '';
						html_produk = `
						<table class='table' style='font-size:12px;border:0'>
							<tr style="list-style:none;background-color: transparent !important;"> 
								<td style="padding:7px;border-right:none;border-left:none;color:#000"><b>Nama Item </td>
								<td style="padding:7px;border-right:none;border-left:none;color:#000;width:1%">:</td> </b>
				...
				if (user != 'PPIC' && user != 'AP'  )
				{
					...
					item_tambahan = `
						<tr style="background:#f2f2f2" id="item_tambahan${ rowNum }">
							...
							<td colspan="6">			
								<textarea class="form-control" name="eta_ket[${ rowNum }]" id="eta_ket${ rowNum }" placeholder="KET. ETA" rows="3" style="color:#dc3545;font-weight:bold;resize:none"></textarea>
							</td>
						</tr>	
					`;
