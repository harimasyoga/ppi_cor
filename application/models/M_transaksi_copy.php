        ...
        foreach ($params->id_produk as $key => $value) 
        {
            ...
            $data = array(
             ...
             if ($status == 'insert') {
                ...
                $result= $this->db->update("trs_po_detail",$data,array(
                                                                    'no_po' => $params->no_po,
                                                                    'kode_mc' => $produk->kode_mc
                                                                )
                                          );
        ...
        foreach ($params->id_produk as $key => $value) 
        {
            ...
            $data = array(
             ...
             if ($status == 'insert') {
                ...
                $this->db->set("edit_user", $this->username);
                $this->db->set("edit_time", date('Y-m-d H:i:s'));
                $result= $this->db->update("trs_so_detail",$data,array(
                                                                    'no_so' => $params->no_so
                                                                )
                                          );
             }
        ...
        if (!empty($params->no_so)) {
            ...
            $data_detail = array(
             ...
             if ($status == 'insert') {
                ...
                $this->db->set("edit_user", $this->username);
                $this->db->set("edit_time", date('Y-m-d H:i:s'));
                $result= $this->db->update("trs_wo",$data_update,array(
                                                                    'no_wo' => $params->no_wo
                                                                )
                                          );
                ...
                $this->db->set("edit_user", $this->username);
                $this->db->set("edit_time", date('Y-m-d H:i:s'));
                $result= $this->db->update("trs_wo_detail",$data_detail,array(
                                                                    'no_wo' => $params->no_wo
                                                                )
                                          );
             }
