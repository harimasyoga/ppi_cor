...
class Logistik extends CI_Controller
{
	...
	function cariLaporanSJLaminasi()
	{
		...
		($cv_attn != "") ? $cvAttn = "AND p.id_hub='$cv_attn'" : $cvAttn = '';
		if($tgl1 == $tgl2){
		...
		($this->session->userdata('username') == 'usman') ? $where3 = "AND p.jenis_lm='PEKALONGAN'" : $where3 = "AND p.jenis_lm='PPI'";
		// HARI
		if($jenis == 'HARI'){
			...
			INNER JOIN trs_po_lm p ON r.rk_no_po=p.no_po_lm
			WHERE r.rk_status='Close' AND l.tgl BETWEEN '$tgl1' AND '$tgl2' $wcust $wattn $cvAttn $where $where2
		...
		if($jenis == 'CUSTOMER'){
			...
			INNER JOIN m_rk_laminasi r ON r.id_pl_lm=l.id AND r.rk_urut=l.no_pl_urut AND r.rk_no_po=l.no_po AND r.rk_tgl=l.tgl AND r.id_pelanggan_lm=l.id_perusahaan
			INNER JOIN trs_po_lm p ON r.rk_no_po=p.no_po_lm
			WHERE r.rk_status='Close' AND l.tgl BETWEEN '$tgl1' AND '$tgl2' $wcust $wattn $cvAttn $where $where3
	...
	function pilihSJInvLam()
	{
		...
		if($no_surat == ''){
			...
			($this->session->userdata('username') == 'usman') ? $where = "AND o.jenis_lm='PEKALONGAN'" : $where = "AND o.jenis_lm='PPI'";
	...
	function cariLaporanLaminasi()
	{
		...
		WHERE h.tgl_surat_jalan BETWEEN '$tgl1_lap' AND '$tgl2_lap' AND h.jenis_lm LIKE '%$pilih%' $wcust $wattn
	...
	function cariPembayaranLaminasi()
	{
		...
		if($opsi == 'laporan'){
			...
			$plh_cust = $_GET["plh_cust"];
			$attn = $_GET["attn"];
			$byr_attn = $_GET["byr_attn"];
			$tgl1_jt = $_GET["tgl1_jt"];
			$tgl2_jt = $_GET["tgl2_jt"];
		}
		($plh_cust == "") ? $wcust = "" : $wcust = "AND h.id_pelanggan_lm='$plh_cust' AND h.attn_lam_inv='$attn'";
		($byr_attn == "") ? $wattn = "" : $wattn = "AND h.bank='$byr_attn'";
		$header = $this->db->query("SELECT*FROM invoice_laminasi_header h
		WHERE h.acc_owner='Y' AND h.tgl_jatuh_tempo BETWEEN '$tgl1_jt' AND '$tgl2_jt' AND h.jenis_lm LIKE '%$plh_pilih%' AND h.status_bayar LIKE '%$plh_bayar%' $wcust $wattn
	...
	function load_data_1()
	{
		...
		}else if($jenis=='spill')
		{
			...
			$queryd   = "SELECT*FROM invoice_detail where no_invoice='$no' ORDER BY TRIM(no_surat) ";
		}else if($jenis=='spill_inv_beli')
		{
			$queryh   = "SELECT IFNULL(
			(select sum(jumlah_bayar) from trs_bayar_inv_beli t
						where t.no_inv_beli=a.no_inv_beli
						group by no_inv_beli),0) jum_bayar,
						a.*,b.nm_hub,c.nm_supp FROM invoice_header_beli a
		...
		{ 
			...
			FROM trs_po_bhnbk a 
			JOIN m_hub b ON a.hub=b.id_hub where id_po_bhn='$id' ";
		}else if($jenis=='spill_stok')
		{ 
			$queryh   = "SELECT*FROM trs_h_stok_bb a
			JOIN trs_d_stok_bb b ON a.no_stok=b.no_stok
			JOIN m_hub c ON b.id_hub=c.id_hub 
			where status='Open' and id_stok_d='$id'
			ORDER BY tgl_stok desc,a.no_stok,id_stok ";
			
			$queryd   = "SELECT*FROM trs_h_stok_bb a
			JOIN trs_d_stok_bb b ON a.no_stok=b.no_stok
			JOIN m_hub c ON b.id_hub=c.id_hub 
			where status='Open' and id_stok_d='$id'
			ORDER BY tgl_stok desc,a.no_stok,id_stok ";
	...
	function get_foto_bc()
	{
		...
		$header   = $this->db->query("SELECT *,cek_bc AS cekinv, img_bc AS inpinv FROM $jenis where $field ='$no' ")->row();
	...
	function get_foto_faktur()
	{
		...
		$header   = $this->db->query("SELECT *, cek_faktur AS cekinv, img_faktur AS inpinv FROM $jenis where $field ='$no' ")->row();
	...
	function get_foto_resi()
	{
		...
		$header   = $this->db->query("SELECT *, cek_resi AS cekinv, img_resi AS inpinv FROM $jenis where $field ='$no' ")->row();
	...
	function get_foto_inv_terima()
	{
		...
		$header   = $this->db->query("SELECT *, cek_inv_terima AS cekinv, img_inv_terima AS inpinv FROM $jenis where $field ='$no' ")->row();
	...
	function get_foto_mutasi()
	{
		...
		$header   = $this->db->query("SELECT *, cek_mutasi AS cekinv, img_mutasi AS inpinv FROM $jenis where $field ='$no' ")->row();
	...
	function get_foto_sj_balik()
	{
		...
		$header   = $this->db->query("SELECT *, cek_sj_balik AS cekinv, img_sj_balik AS inpinv FROM $jenis where $field ='$no' ")->row();
	...
	function save_sj_balik()
	{
		...
		if($cek_data->img_sj_balik != '' || $cek_data->img_sj_balik != null ){
			// Hapus File Foto
			unlink("assets/gambar_inv_sj_balik/".$cek_data->img_sj_balik);
		}

		/* LOGO */
		//$nmfile = "file_".time(); //nama file saya beri nama langsung dan diikuti fungsi time
		$config['upload_path']   = './assets/gambar_inv_sj_balik/'; //path folder
		$config['allowed_types'] = 'jpg|png|jpeg|pdf'; //type yang dapat diakses bisa anda sesuaikan
		// $config['max_size']      = 1024; //maksimum besar file 2M
		// $config['max_width']     = 'none'; //lebar maksimum 1288 px
		// $config['max_height']    = 'none'; //tinggi maksimu 768 px
		...
		if($_FILES['filefoto']['name'] && $params->tgl_blk != '')
		{
			if ($this->upload->do_upload('filefoto'))
			{
				$gbrBukti = $this->upload->data();
	...
	function save_upload_inv()
	{
		...
		/* LOGO */
		//$nmfile = "file_".time(); //nama file saya beri nama langsung dan diikuti fungsi time
		$config['upload_path']   = './assets/gambar_inv_upload_inv/'; //path folder
		$config['allowed_types'] = 'jpg|png|jpeg|pdf'; //type yang dapat diakses bisa anda sesuaikan
		// $config['max_size']      = 1024; //maksimum besar file 2M
		// $config['max_width']     = 'none'; //lebar maksimum 1288 px
		// $config['max_height']    = 'none'; //tinggi maksimu 768 px
	...
	function cariLapExpired()
	{
		...
		if($tgl_exp == '' && $ex_pilih != 'SEMUA'){
			...
			// GROUP SALES
			$sales = $this->db->query("SELECT s.id_sales,s.nm_sales FROM invoice_header h
			INNER JOIN m_pelanggan p ON h.id_perusahaan=p.id_pelanggan
			INNER JOIN m_sales s ON p.id_sales=s.id_sales
			WHERE h.type!='roll' AND h.status_inv='Xp' $wP $wSales
			GROUP BY s.nm_sales,s.id_sales
			UNION
			SELECT s.id_sales,s.nm_sales FROM invoice_header h
			...
			if($sales->num_rows() == 0){
				...
				$html .= '<table style="color:#000">';
					foreach($sales->result() as $s){
						$html .= '<tr>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px">NO.</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px">TGL INVOICE</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px">TYPE</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px">NO INVOICE</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px">NO SURAT</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px">NM PERUSAHAAN</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px 23px;text-align:center">B C</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px;text-align:center">FAKTUR</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px;text-align:center">NO. RESI</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px;text-align:center">I. TERIMA</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px;text-align:center">MUTASI</th>
							<th style="background:#000;border:1px solid #666;color:#fff;padding:6px;text-align:center">SJ BALIK</th>
						</tr>
						<tr>
							<td style="padding:6px;font-weight:bold;background:#ccc;border:1px solid #000" colspan="12">'.$s->nm_sales.'</td>
						</tr>';
						$header = $this->db->query("SELECT s.id_sales,s.nm_sales,h.*,d.* FROM invoice_header h
							INNER JOIN m_pelanggan p ON h.id_perusahaan=p.id_pelanggan
							INNER JOIN m_sales s ON p.id_sales=s.id_sales
							INNER JOIN invoice_detail d ON h.no_invoice=d.no_invoice
							WHERE h.type!='roll' AND h.status_inv='Xp' AND s.id_sales='$s->id_sales' $wP
							GROUP BY h.no_invoice
							UNION
							SELECT s.id_sales,s.nm_sales,h.*,d.* FROM invoice_header h
							INNER JOIN m_perusahaan p ON h.id_perusahaan=p.id
							INNER JOIN m_sales s ON p.id_sales=s.id_sales
							INNER JOIN invoice_detail d ON h.no_invoice=d.no_invoice
							WHERE h.type='roll' AND h.status_inv='Xp' AND s.id_sales='$s->id_sales' $wP
	...
	function chPiuTahun()
	{
		...
		$qTahun = $this->db->query("SELECT YEAR(a.tgl_invoice) AS tahun FROM invoice_header a
		WHERE a.jml_mutasi IS NOT NULL AND a.tgl_invoice > '2025-01-01'
		GROUP BY YEAR(a.tgl_invoice)");

		$html .= '<option value="">PILIH</option>';
		$html .= '<option value="ALL">SEMUA</option>';
		foreach($qTahun->result() as $r){
	...
	function listPiutang()
	{
		...
		if($tahun == "" && $bulan == ""){
			$html .= '<div style="font-weight:bold">PILIH TAHUN & BULAN!</div>';
		}else if($tahun != "ALL" && $bulan == ""){
			$html .= '<div style="font-weight:bold">PILIH BULAN!</div>';
		}else{
			if($tahun == "ALL"){
				$wTanggal = "'2025-01-01' AND '9999-01-01'";
			}else{
				($bulan != 'ALL') ? $wTanggal = "'$tahun-$bulan-01' AND '$tahun-$bulan-31'" : $wTanggal = "'$tahun-01-01' AND '$tahun-12-21'";
			}
			$sales = $this->db->query("SELECT s.id_sales,h.type,s.nm_sales,SUM(h.jml_mutasi) AS jml_mutasi FROM invoice_header h
			INNER JOIN m_pelanggan p ON h.id_perusahaan=p.id_pelanggan
			INNER JOIN m_sales s ON p.id_sales=s.id_sales
			WHERE h.tgl_invoice BETWEEN $wTanggal AND h.type!='ROLL' AND h.jml_mutasi IS NOT NULL AND h.acc_owner='N' $wIdSls
			GROUP BY s.nm_sales,s.id_sales
			UNION
			...
			if($sales->num_rows() == 0){
				$html .= '<div style="font-weight:bold">DATA KOSONG!</div>';
			}else if($sales->num_rows() != 0){
				// LATESET
				$qLatUpd = $this->db->query("SELECT*FROM latest_update WHERE jenis='LAPORAN PIUTANG' ORDER BY id DESC");
				if($qLatUpd->num_rows() != ''){
					$xLatUpd = strtoupper($this->m_fungsi->getHariIni(substr($qLatUpd->row()->update_at,0,10))).', '.strtoupper($this->m_fungsi->tanggal_format_indonesia(substr($qLatUpd->row()->update_at,0,10))).' '.substr($qLatUpd->row()->update_at,11);
				}else{
					$xLatUpd = '-';
				}
				// BUTTON UPDATE
				if($lvl == 'Admin' || ($lvl == 'Keuangan1' && $uName == 'bumagda')){
					$bLatUpd = ' <button type="button" class="btn btn-xs btn-primary" style="font-weight:bold" onclick="btnLatestUpdate()">update</button>';
				}else{
					$bLatUpd = '';
				}
				$html .= '<div style="margin-bottom:6px;font-weight:bold">
					<div>LATEST UPDATE : '.$xLatUpd.$bLatUpd.'</div>
				</div>';
				$html .= '<div style="margin-bottom:4px;display:block">
					<span class="bg-primary" style="vertical-align:top;font-weight:bold;padding:2px 4px;font-size:14px">INVOICE</span><span class="bg-warning" style="vertical-align:top;font-weight:bold;padding:2px 4px;font-size:14px">REMINDER</span><span class="bg-danger" style="vertical-align:top;font-weight:bold;padding:2px 4px;font-size:14px">JATUH TEMPO</span><span class="bg-success" style="vertical-align:top;font-weight:bold;padding:2px 4px;font-size:14px">PEMBAYARAN</span>
				</div>';
				$html .= '<table style="color:#000;border-collapse: collapse">';
					$html .= '<tr>
						<td style="background:#ccc;padding:5px;border:1px solid #aaa;font-weight:bold" colspan="3">SALES/CUSTOMER</td>
						<td style="background:#ccc;padding:5px 10px;border:1px solid #aaa;font-weight:bold;text-align:center">PIUTANG (sj awal)</td>
						<td style="background:#ccc;padding:5px 10px;border:1px solid #aaa;font-weight:bold;text-align:center">PIUTANG (invoice)</td>
					...
					foreach($sales->result() as $s){
						if($s->type == 'roll'){
							...
							$wh1 = "AND h.tgl_invoice BETWEEN $wTanggal AND h.type='ROLL'";
						}else{
							$se9 = "p.attn";
							$se1 = "p.nm_pelanggan";
							$n666 = "''";
						...
						// DRAFT INVOICE
						$piuDraftJt = $this->db->query("SELECT SUM(h.jml_mutasi) AS jml_mutasi_jt FROM invoice_draft_header h
							$in1
							INNER JOIN m_sales s ON p.id_sales=s.id_sales
							INNER JOIN invoice_header h2 ON h.no_invoice=h2.no_invoice
							WHERE h2.acc_owner='N' AND h.jml_mutasi IS NOT NULL AND s.id_sales='$s->id_sales' $wh1
							GROUP BY s.id_sales
						");
						($piuDraftJt->num_rows() != 0) ? $xPiuDrfatJt = $piuDraftJt->row()->jml_mutasi_jt : $xPiuDrfatJt = 0;
						// PIUTANG JT SALES
						$piuSalJt = $this->db->query("SELECT SUM(h.jml_mutasi) AS jml_mutasi_jt FROM invoice_header h
							...
							AND s.id_sales='$s->id_sales' AND ((h.status_inv='Xp' AND h.img_inv_terima IS NOT NULL) OR (h.status_inv='Xp' AND h.img_mutasi IS NOT NULL))
							GROUP BY s.id_sales
						");
						...
						// COUNT REMINDER SALES
						$cRR = $this->db->query("SELECT s.id_sales,COUNT(h.no_invoice) AS cnt FROM invoice_header h
							$in1
							INNER JOIN m_sales s ON p.id_sales=s.id_sales
							WHERE s.id_sales='$s->id_sales' AND h.status_inv='Open' AND h.jml_mutasi IS NOT NULL AND h.img_inv_terima IS NOT NULL AND h.acc_owner='N'
							AND (DATEDIFF(h.tgl_jatuh_tempo , h.tgl_invoice) - DATEDIFF(CURDATE(), SUBSTR(h.inp_inv_terima, 1, 10))) IN ('0', '1', '2', '3', '4', '5') $wh1
							GROUP BY s.id_sales
						");
						($cRR->num_rows() != 0) ? $xCrR = '<span class="bg-warning" style="vertical-align:top;font-weight:bold;padding:2px 4px;font-size:12px">'.$cRR->row()->cnt.'</span>' : $xCrR = '';
						// COUNT BAYAR
						$c4h = $this->db->query("SELECT s.id_sales,COUNT(h.no_invoice) AS cnt FROM invoice_header h
							$in1
							INNER JOIN m_sales s ON p.id_sales=s.id_sales
							INNER JOIN invoice_bayar b ON h.no_invoice=b.no_invoice
							WHERE s.id_sales='$s->id_sales' AND h.jml_mutasi IS NOT NULL AND h.img_mutasi IS NOT NULL AND h.acc_owner='N' $wh1
							GROUP BY s.id_sales
						");
						($c4h->num_rows() != 0) ? $t4h = '<span class="bg-success" style="vertical-align:top;font-weight:bold;padding:2px 4px;font-size:12px">'.$c4h->row()->cnt.'</span>' : $t4h = '';
						// ISI
						$html .= '<tr class="tr0">
							<td style="background:#eee;border:1px solid #aaa;font-weight:bold;padding:5px" colspan="3">
								...
								'.$s->nm_sales.$xCv.$xCrR.$xXxP.$t4h.'
							</td>
							<td style="background:#eee;border:1px solid #aaa;font-weight:bold;padding:5px;text-align:right">'.number_format($xPiuDrfatJt, 0, ',', '.').'</td>
						...
						// CUSTOMER
						$cust = $this->db->query("SELECT s.id_sales,h.type,h.id_perusahaan,$se9 AS attn,$se1 AS nm_pelanggan,$n666 AS note,SUM(h.jml_mutasi) AS jml_mutasi FROM invoice_header h
							$in1
							INNER JOIN m_sales s ON p.id_sales=s.id_sales
						...
						if($cust->num_rows() != 0){
							foreach($cust->result() as $r){
								// DRAFT INVOICE
								$piuCusDraftJt = $this->db->query("SELECT SUM(h.jml_mutasi) AS jml_mutasi_jt FROM invoice_draft_header h
									$in1
									INNER JOIN m_sales s ON p.id_sales=s.id_sales
									INNER JOIN invoice_header h2 ON h.no_invoice=h2.no_invoice
									WHERE h2.acc_owner='N' AND h.jml_mutasi IS NOT NULL $wh1 AND s.id_sales='$r->id_sales' AND h.id_perusahaan='$r->id_perusahaan'
									GROUP BY s.id_sales,h.id_perusahaan
								");
								($piuCusDraftJt->num_rows() != 0) ? $xPiuCusDJt = $piuCusDraftJt->row()->jml_mutasi_jt : $xPiuCusDJt = 0;
								// PIUTANG JT CUSTOMER
								$piuCusJt = $this->db->query("SELECT SUM(h.jml_mutasi) AS jml_mutasi_jt FROM invoice_header h
									...
									AND s.id_sales='$r->id_sales' AND h.id_perusahaan='$r->id_perusahaan' AND ((h.status_inv='Xp' AND h.img_inv_terima IS NOT NULL) OR (h.status_inv='Xp' AND h.img_mutasi IS NOT NULL))
									GROUP BY s.id_sales,h.id_perusahaan
								");
								...
								// COUNT REMINDER
								$c3 = $this->db->query("SELECT s.id_sales,COUNT(h.no_invoice) AS cnt FROM invoice_header h
									$in1
									INNER JOIN m_sales s ON p.id_sales=s.id_sales
									WHERE s.id_sales='$s->id_sales' AND h.id_perusahaan='$r->id_perusahaan' AND h.status_inv='Open' AND h.jml_mutasi IS NOT NULL AND h.img_inv_terima IS NOT NULL AND h.acc_owner='N'
									AND (DATEDIFF(h.tgl_jatuh_tempo , h.tgl_invoice) - DATEDIFF(CURDATE(), SUBSTR(h.inp_inv_terima, 1, 10))) IN ('0', '1', '2', '3', '4', '5') $wh1
									GROUP BY s.id_sales,h.id_perusahaan
								");
								($c3->num_rows() != 0) ? $t3 = '<span class="bg-warning" style="vertical-align:top;font-weight:bold;padding:2px 4px;font-size:12px">'.$c3->row()->cnt.'</span>' : $t3 = '';
								// COUNT BAYAR
								$c4 = $this->db->query("SELECT s.id_sales,COUNT(h.no_invoice) AS cnt FROM invoice_header h
									$in1
									INNER JOIN m_sales s ON p.id_sales=s.id_sales
									INNER JOIN invoice_bayar b ON h.no_invoice=b.no_invoice
									WHERE s.id_sales='$s->id_sales' AND h.id_perusahaan='$r->id_perusahaan' AND h.jml_mutasi IS NOT NULL AND h.img_mutasi IS NOT NULL AND h.acc_owner='N' $wh1
									GROUP BY s.id_sales,h.id_perusahaan
								");
								($c4->num_rows() != 0) ? $t4 = '<span class="bg-success" style="vertical-align:top;font-weight:bold;padding:2px 4px;font-size:12px">'.$c4->row()->cnt.'</span>' : $t4 = '';
								($r->attn == "-" || $r->attn == "") ? $atZ = '' : $atZ = '<div style="padding-left:25px">'.$r->attn.'</div>';
								($r->note == "" || $r->note == null) ? $atN = '' : $atN = ' ('.$r->note.')';
								($r->type == 'roll') ? $pt1 = $r->id_perusahaan * 9 : $pt1 = $r->id_perusahaan;
								// ISI
								$html .= '<tr class="tr1 t'.$r->id_sales.'" style="display:none">
									<td style="background:#ddd;border:1px solid #aaa;font-weight:bold;padding:5px 5px 5px 15px" colspan="3">
										...
										'.$r->nm_pelanggan.$atN.$t1.$t3.$t2.$t4.$atZ.'
									</td>
									<td style="background:#ddd;border:1px solid #aaa;vertical-align:top;font-weight:bold;padding:5px;text-align:right">'.number_format($xPiuCusDJt, 0, ',', '.').'</td>
								...
								if($noInvNoSj->num_rows() != 0){
									...
									foreach($noInvNoSj->result() as $n){
										$l++;
										// DRAFT INVOICE
										$piuNoInvDNoSj = $this->db->query("SELECT SUM(h.jml_mutasi) AS jml_mutasi_jt FROM invoice_draft_header h
											$in1
											INNER JOIN m_sales s ON p.id_sales=s.id_sales
											INNER JOIN invoice_header h2 ON h.no_invoice=h2.no_invoice
											WHERE h2.acc_owner='N' AND h.jml_mutasi IS NOT NULL $wh1 AND s.id_sales='$n->id_sales' AND h.id_perusahaan='$n->id_perusahaan' AND h.no_invoice='$n->no_invoice'
											GROUP BY s.id_sales,h.id_perusahaan,h.no_invoice
										");
										($piuNoInvDNoSj->num_rows() != 0) ? $xPiuNoInvDNoSj = $piuNoInvDNoSj->row()->jml_mutasi_jt : $xPiuNoInvDNoSj = 0;
										// PIUTANG JT NO INVOICE DAN SJ
										$piuNoInvNoSj = $this->db->query("SELECT SUM(h.jml_mutasi) AS jml_mutasi_jt FROM invoice_header h
											$in1
											INNER JOIN m_sales s ON p.id_sales=s.id_sales
											WHERE h.acc_owner='N' AND h.jml_mutasi IS NOT NULL $wh1
											AND s.id_sales='$n->id_sales' AND h.id_perusahaan='$n->id_perusahaan' AND h.no_invoice='$n->no_invoice' AND ((h.status_inv='Xp' AND h.img_inv_terima IS NOT NULL) OR (h.status_inv='Xp' AND h.img_mutasi IS NOT NULL))
											GROUP BY s.id_sales,h.id_perusahaan,h.no_invoice
										");
										...
										// TIPE DD
										($n->type == 'roll') ? $pt2 = $n->id_perusahaan * 9 : $pt2 = $n->id_perusahaan;
										// BTN SAKTI
										if($lvl == 'Admin'){
										...
										($hoursH == 0) ? $tHoursH = '' : $tHoursH = $hoursH.'h. ';
										($minutesH == 0) ? $tMinutesH = '' : $tMinutesH = $minutesH.'m.';
										$tXtsiSaharI = $tHoursH.$tMinutesH;
										$qPay = $this->db->query("SELECT*FROM invoice_bayar WHERE no_invoice='$n->no_invoice'");
										if($qPay->num_rows() != 0){
											if($qPay->row()->jumlah == $n->jml_mutasi){
												$iTxTpY = 'success'; $xTxTpY = 'LUNAS';
											}else{
												$iTxTpY = 'dark'; $xTxTpY = 'BELUM LUNAS';
											}
										}else{
											$iTxTpY = ''; $xTxTpY = '';
										}
										if($n->status_inv == 'Xp'){
											if($qPay->num_rows() == 0 && $sisaHari < 0){
												$cekMutmut = ' <span class="bg-primary" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:2px 0 0 2px">'.$n->tempo.'</span><span class="bg-danger" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:0 2px 2px 0">+'.str_replace("-", "", $sisaHari). ' HARI</span>';
											}else if($qPay->num_rows() != 0 && $sisaHari < 0){
												$cekMutmut = ' <span class="bg-primary" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:2px 0 0 2px">'.$n->tempo.'</span><span class="bg-danger" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:0">+'.str_replace("-", "", $sisaHari). ' HARI</span><span class="bg-'.$iTxTpY.'" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:0 2px 2px 0">'.$xTxTpY.'</span>';
											}else{
												$cekMutmut = '';
											}
										}else{
											if($qPay->num_rows() != 0){
												$iBrRdi = '0';
												$ttxTlNs = '<span class="bg-'.$iTxTpY.'" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:0 2px 2px 0">'.$xTxTpY.'</span>';
												$ttxTlNs2 = '<span class="bg-'.$iTxTpY.'" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:2px">'.$xTxTpY.'</span>';
											}else{
												$iBrRdi = '0 2px 2px 0'; $ttxTlNs = ''; $ttxTlNs2 = '';
											}
											// KET REMIND
											if($sisaHari <= 5 && $sisaHari > 0){
												$cekMutmut = ' <span class="bg-primary" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:2px 0 0 2px">'.$n->tempo.'</span><span class="bg-warning" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:'.$iBrRdi.'">-'.str_replace("-", "", $sisaHari). ' HARI</span>'.$ttxTlNs;
											}else if($sisaHari == 0){
												$cekMutmut = ' <span class="bg-primary" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:2px 0 0 2px">'.$n->tempo.'</span><span class="bg-warning" style="vertical-align:top;padding:2px 4px;font-size:12px;border-radius:'.$iBrRdi.'">'.$tXtsiSaharI.'</span>'.$ttxTlNs;
											}else{
												$cekMutmut = ' '.$ttxTlNs2;
											}
										}
										...
										$html .= '<tr class="tr2 c'.$pt2.' m-2" style="display:none">
											...
											<td style="border:1px solid #aaa;border-width:1px 0;padding:5px 0">'.$cekMutmut.'</td>
											<td style="border:1px solid #aaa;border-width:1px 0;padding:5px;text-align:right">'.number_format($xPiuNoInvDNoSj, 0, ',', '.').'</td>
										...
										if($qPay->num_rows() != 0){
											...
											foreach($qPay->result() as $p){
												...
												$html .= '<tr class="tr2 c'.$pt2.' m-2" style="display:none">
													<td style="padding:5px;border-left:1px solid #aaa" colspan="4"></td>
											...
											if($seLisiH < 0){
												$html .= '<tr class="tr2 c'.$pt2.' m-2" style="display:none">
													<td style="border-left:1px solid #aaa;padding:5px" colspan="4"></td>
					...
					// TOTAL
					$html .= '<tr>
						<td style="background:#ccc;padding:5px;border:1px solid #aaa" colspan="3"></td>
						<td style="background:#ccc;padding:5px;border:1px solid #aaa;font-weight:bold;text-align:right">'.number_format($sumDft, 0, ',', '.').'</td>
	...
	{
		$result = $this->m_logistik->btnLatestUpdate();
		echo json_encode($result);
	}
	...
	function load_data()
	{
		...
		if ($jenis == "Invoice") {
			...
			foreach ($query as $r) {
				...
				if($r->inp_sj_balik != null && $r->tgl_sj_blk == null){
					$tglBoxNRoll = strtotime(substr($r->inp_sj_balik, 0, 10));
				}else if($r->inp_sj_balik != null && $r->tgl_sj_blk != null){
				...
				$expired6              = strtotime($r->tgl_invoice) + (144*60*60);
				$actualDate6           = time();
				...
				if($r->pajak == 'nonppn' || $r->type == 'roll' || $peTe == 0){
					...
					if($r->img_bc=='' || $r->inp_sj_balik == null){
						if($r->inp_sj_balik == null){
				...
				if($r->pajak == 'nonppn'){
					...
					if($r->img_faktur=='' || $r->inp_sj_balik == null) {
						if($r->inp_sj_balik == null){
				...
				// RESI
				if($r->img_resi=='' || $r->inp_sj_balik == null){
					if($r->inp_sj_balik == null){
				...
				// INVOICE DITERIMA
				if($r->img_inv_terima=='' || $r->inp_sj_balik == null){
					if($r->inp_sj_balik == null){
				...
				if($r->jml_mutasi != null){
					...
					$cekDraft = $this->db->query("SELECT*FROM invoice_draft_header WHERE no_invoice='$r->no_invoice' ORDER BY id ASC LIMIT 1");
				...
				// UPDATE JML NOMINAL DI INVOICE HEADER BELUM DI ACC
				if($r->acc_owner != 'Y'){
					$this->db->query("UPDATE invoice_header SET jml_mutasi='$total' WHERE no_invoice='$r->no_invoice' AND id='$r->id'");
				}
				$row[] = '<div class="text-right"><b>'.number_format($total, 0, ",", ".").$txtSel.'</b></div>';
				// PEMBAYARAN
				if($r->acc_owner == "Y") {
					if($r->jml_mutasi != null){
						if($sumBayar->jumlah == null){
							$txtB = 'btn-warning'; $txtT = 'BELUM BAYAR';
						}else{
							if($seLisiH < 0){
								$txtB = 'btn-light'; $txtT = 'DI CICIL';
							}else{
				...
				if (in_array($this->session->userdata('username'), ['karina','tegar'])){
					...
					$urll1 = ''; $urll2 = "onclick=acc_inv('$r->no_invoice','$r->acc_owner')";
				} else {
					$urll1 = ''; $urll2 = '';
				}
				$row[] = '<div class="text-center">
			...
			foreach ($query as $r) {
				...
				{
					$urll2 = "onclick=acc_inv('$r->id_bayar_inv','$r->acc_owner')";
				} else {
					$urll2 = '';
				}
			...
			$query = $this->db->query("SELECT *,a.id_bayar_inv as id_ok,a.acc_owner 
			FROM trs_bayar_inv_beli a 
			join invoice_header_beli b on a.no_inv_beli=b.no_inv_beli 
			join m_hub c on b.id_hub=c.id_hub
			join m_supp d on b.id_supp=d.id_supp
			...
			$i               = 1;
			foreach ($query as $r) {				
				// $result_transaksi = $this->db->query("SELECT *,
				// (
				// SELECT nm FROM(
				// 			select kd_akun as kd,nm_akun as nm,jenis,dk from m_kode_akun
				// 			union all
				...
				// FROM invoice_detail_beli WHERE no_inv_beli='$r->no_inv_beli' order by id_det_beli");
				// if($result_transaksi->num_rows() == '1'){
				// 	$transaksi = $result_transaksi->row()->transaksi;
				// }else{					
				// 	$transaksi_result    = '';
				// 	foreach($result_transaksi->result() as $row){
				// 		$transaksi_result .= '<b>- </b>'.$row->transaksi.'<br>';
				// 	}
				// 	$transaksi = $transaksi_result;
				// }				
				...
				<table>
					<tr style="background-color: transparent !important">
						<td style="padding : 2px;border:none;"><b>NO INV </td>
						<td style="padding : 2px;border:none;">:</td></b> 
						<td style="padding : 2px;border:none;">'.$r->no_inv_beli .'<br></td>
					</tr>
					<tr style="background-color: transparent !important">
						<td style="padding : 2px;border:none;"><b>TGL INV </td>
						<td style="padding : 2px;border:none;">:</td></b> 
						<td style="padding : 2px;border:none;">'.$r->tgl_inv .'<br></td>
					</tr>
					<tr style="background-color: transparent !important">
						<td style="padding : 2px;border:none;"><b>HUB</td>
						<td style="padding : 2px;border:none;">:</td></b> 
						<td style="padding : 2px;border:none;">'.$r->nm_hub .'<br></td>
					</tr>
					<tr style="background-color: transparent !important">
						<td style="padding : 2px;border:none;"><b>TAX</td>
						<td style="padding : 2px;border:none;">:</td></b> 
						<td style="padding : 2px;border:none;">'.$pajak .'<br></td>
					</tr>
					<tr style="background-color: transparent !important">
						<td style="padding : 2px;border:none;"><b>SUPPLIER</td>
						<td style="padding : 2px;border:none;">:</td></b> 
						<td style="padding : 2px;border:none;">'.$r->nm_supp .'<br></td>
					</tr>
					';
				$row[] = '<div class="text-center">'.$r->tgl_bayar.'</div>';
				$row[] = '<div class="text-right"><b>'.number_format($r->total_inv, 0, ",", ".").'</b></div>';
		...
		}else if ($jenis == "stok_bb_rinci") {			
			$query = $this->db->query("SELECT*FROM trs_h_stok_bb a
			JOIN trs_d_stok_bb b ON a.no_stok=b.no_stok
			JOIN m_hub c ON b.id_hub=c.id_hub 
			where status='Open' AND id_stok_d not in (select id_stok_d from invoice_bhn)
			ORDER BY tgl_stok ,a.no_stok,id_stok")->result();

			$i               = 1;
			foreach ($query as $r) {

				$id           = "'$r->id_stok'";
				$id_stok_d    = "'$r->id_stok_d'";
				$no_stok      = "'$r->no_stok'";
				$no_stok2     = "$r->no_stok";
				...
				$row[]          = '<div >'.$r->no_timbangan.'</div>';
				$row[]          = '<div class="text-center">'.number_format($r->total_timb, 0, ",", ".").' Kg</div>' ;
				$row[]          = '<div class="text-center">'.number_format($r->datang_bhn_bk, 0, ",", ".").' Kg</div>' ;
				$row[]          = '<div class="text-center">'.$r->nm_hub.'</div>';
				
				$aksi = '
						<button type="button" title="PILIH"  onclick="spilldata(' . $id_stok_d . ',' . $no_stok . ')" class="btn btn-success btn-sm">
							<i class="fas fa-check-circle"></i>
						</button> 
						';
			...
			$query = $this->db->query("SELECT*from invoice_bhn a
			join m_hub b on a.id_hub=b.id_hub
			...
			foreach ($query as $r) {
				...
				$row[]  = '<div class="text-center">'.$r->no_stok.'</div>';
			...
			foreach ($query as $r) {
				...
				$row[] = '<div class="text-center">
					<button type="button" class="btn btn-xs '.$txtB.'" style="font-weight:bold" onclick="editInvoiceLaminasi('."'".$r->id."'".','."'detail'".')">'.$txtT.'</button>
				</div>';
				// TOTAL
				$row[] = '<div class="text-right" style="font-weight:bold;color:#000">'.number_format($total_disc,0,',','.').$kurengByr.'</div>';
				// ADMIN
				$row[] = '<div class="text-center">
					<div class="dropup">
						<button class="dropbtn btn btn-sm btn-success"><i class="fas fa-check-circle" onclick="editInvoiceLaminasi('."'".$r->id."'".','."'detail'".')"></i></button>
				...
				$row[] = '<div class="text-center">
					<div class="dropup">
						<button class="dropbtn btn btn-sm '.$bt2.'" '.$p2.' onclick="editInvoiceLaminasi('."'".$r->id."'".','."'detail'".')"><i '.$fa2.'></i></button>
						<div class="dropup-content">
	...
	function loadDataGDCorr()
	{
		...
		foreach ($query as $r) {
			...
			if($qq->num_rows() != 0){
				$vSa = ($qq->row($hari.'_stok_awal') == 0) ? '-' : number_format($qq->row($hari.'_stok_awal'),0,',','.');
				$vIn = ($qq->row($hari.'_in') == 0) ? '-' : number_format($qq->row($hari.'_in'),0,',','.');
				$vIn2 = ($qq->row($hari.'_in_rtr') == 0) ? '-' : number_format($qq->row($hari.'_in_rtr'),0,',','.');
				$vOut = ($qq->row($hari.'_out') == 0) ? '-' : number_format($qq->row($hari.'_out'),0,',','.');
				$vOut2 = ($qq->row($hari.'_out_rtr') == 0) ? '-' : number_format($qq->row($hari.'_out_rtr'),0,',','.');
				$vSk = ($qq->row($hari.'_stok_akhir') == 0) ? '-' : number_format($qq->row($hari.'_stok_akhir'),0,',','.');
				$vKet = $qq->row($hari.'_ket');
			}else{
	...
	function plhGCPelanggan()
	{
		...
		if($tgl_awal_cust == ''){
			...
			$html .= '<form role="form" method="post" id="myForm">
				...
				<input type="hidden" id="h_tgl_awal" name="h_tgl_awal" value="'.$tgl_awal_cust.'">
				<div style="padding:0;overflow:auto;white-space:nowrap">
					<table class="table table-bordered table-striped" style="margin:0;border:0">
						<tr>
							<th style="text-align:center;padding:6px">#</th>
						...
						foreach($produk->result() as $r){
							...
							($cekPO->num_rows() != 0) ? $infoPO = '<button type="button" class="btn btn-sm" style="padding:0" onclick="listPO('."'".$id_pelanggan."'".', '."'".$r->id_produk."'".', '."'ADD'".')"><i class="fas fa-info-circle" style="color:#0d6efd"></i></button> ' : $infoPO = '';

							// CEK JIKA ADA KIRIMAN
							$cekKirim = $this->db->query("SELECT*FROM m_rencana_kirim r WHERE r.rk_tgl='$tgl_awal_cust' AND r.id_produk='$r->id_produk' GROUP BY r.id_produk");
							($cekKirim->num_rows() != 0) ? $bb = 'background:#ffa;border:1px solid #dd8;' : $bb = ''; //ffc107
							
							$html .= '<tr style="vertical-align:top">
								<td style="'.$bb.'padding:6px;text-align:center">'.$i.'</td>
								<td style="'.$bb.'padding:6px">'.$dv1.$infoPO.$kat.$r->nm_produk.$dv2.'</td>
								<td style="'.$bb.'padding:6px;text-align:center">'.$uk.'</td>
								<td style="'.$bb.'padding:6px;text-align:center">'.$r->flute.'</td>
								<td style="'.$bb.'padding:6px;text-align:center">'.$this->m_fungsi->kualitas($r->kualitas, $r->flute).'</td>
								<td style="'.$bb.'padding:6px">
	...
	function listPO()
	{
		...
		$po = $this->db->query("SELECT d.qty,p.* FROM trs_po p
		INNER JOIN trs_po_detail d ON p.no_po=d.no_po AND p.kode_po=d.kode_po
		WHERE p.status!='Close' AND p.status_kiriman='Open' AND p.id_pelanggan='$id_pelanggan' AND d.id_produk='$id_produk'
		GROUP BY p.status DESC,p.tgl_po,p.kode_po,d.id_produk");

		$html .= '<div style="font-weight:bold">LIST PO : </div>';
		$html .= '<table>
			<tr style="background:#dee2e6;font-weight:bold;text-align:center">
				<td style="padding:6px 12px;border:1px solid #bbb">#</td>
				<td style="padding:6px;border:1px solid #bbb">TGL. PO</td>
				<td style="padding:6px;border:1px solid #bbb">NO. PO</td>
				<td style="padding:6px;border:1px solid #bbb">QTY PO</td>
				<td style="padding:6px;border:1px solid #bbb">STATUS</td>
			</tr>';
			$i = 0;
			foreach($po->result() as $r){
				$i++;
				($r->status_app3 == 'Y') ? $timeY = ' ('.$r->time_app3.')' : $timeY = '';
				$html .= '<tr>
					<td style="border:1px solid #dee2e6;padding:6px;text-align:center">'.$i.'</td>
					<td style="border:1px solid #dee2e6;padding:6px">'.$r->tgl_po.'</td>
					<td style="border:1px solid #dee2e6;padding:6px">'.$r->kode_po.'</td>
					<td style="border:1px solid #dee2e6;padding:6px;text-align:right">'.number_format($r->qty,0,',','.').'</td>
					<td style="border:1px solid #dee2e6;padding:6px">'.$r->status.$timeY.'</td>
				</tr>';
			}
		$html .= '</table>';

		// KIRIMAN
		$kirim = $this->db->query("SELECT*FROM pl_box p
		INNER JOIN m_rencana_kirim r ON p.id=r.id_pl_box AND p.no_pl_urut=r.rk_urut AND p.tgl=r.rk_tgl
		WHERE p.tgl='$tgl' AND r.id_pelanggan='$id_pelanggan' AND r.id_produk='$id_produk'
		GROUP BY p.tgl,p.no_surat,p.no_po,r.id_pelanggan,r.id_produk");
		if($kirim->num_rows() != 0){
			$html .= '<div style="margin-top:20px;font-weight:bold">PENGIRIMAN : </div>';
			$html .= '<table>
				<tr style="background:#dee2e6;font-weight:bold;text-align:center">
					<td style="padding:6px 12px;border:1px solid #bbb">#</td>
					<td style="padding:6px;border:1px solid #bbb">TGL</td>
					<td style="padding:6px;border:1px solid #bbb">NO. SURAT</td>
					<td style="padding:6px;border:1px solid #bbb">NO. PO</td>
					<td style="padding:6px;border:1px solid #bbb">QTY KIRIM</td>
				</tr>';
				$i = 0;
				$sumQTY = 0;
				foreach($kirim->result() as $k){
					...
					$html .= '<tr>
						<td style="border:1px solid #dee2e6;padding:6px;text-align:center">'.$i.'</td>
						<td style="border:1px solid #dee2e6;padding:6px">'.$k->tgl.'</td>
						<td style="border:1px solid #dee2e6;padding:6px">'.$k->no_surat.'</td>
						<td style="border:1px solid #dee2e6;padding:6px">'.$k->no_po.'</td>
						<td style="border:1px solid #dee2e6;padding:6px;text-align:right">'.number_format($k->qty_muat,0,',','.').'</td>
					</tr>';
					$sumQTY += $k->qty_muat;
				}
				// TOTAL
				if($kirim->num_rows() > 1){
					$html .= '<tr style="background:#dee2e6;font-weight:bold;text-align:center">
						<td style="padding:6px;border:1px solid #bbb;text-align:right" colspan="4">TOTAL</td>
						<td style="padding:6px;border:1px solid #bbb;text-align:right">'.number_format($sumQTY,0,',','.').'</td>
					</tr>';
				}
			$html .= '</table>';
		}
			...
			$html2 .= '<table style="color:#000;border-collapse:collapse">
				<tr>
					...
					for ($i = 1; $i <= $hari; $i++) {
						($i < 10) ? $a = '0'.$i : $a = $i;
						// CEK LIBUR
						$tglLbr = $tahun.'-'.$bulan.'-'.$a;
						$libur = $this->db->query("SELECT*FROM libur WHERE tgl='$tglLbr'");
						$namaHari = date('l', strtotime($tglLbr));
						if($libur->num_rows() != 0){
							$kk = '#fcc';
							$kb = '#ccc';
						}else{
							($namaHari == "Sunday") ? $kk = '#fcc' : $kk = '#f8f9fc';
							($namaHari == "Sunday") ? $kb = '#daa' : $kb = '#ccc';
						}
						$html2 .= '<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center" colspan="8">'.substr($this->m_fungsi->getHariIni($tglLbr),0,3).', '.$a.'</td>';
					}
				$html2 .= '</tr>
				<tr>
					<td style="background:#f8f9fc;padding:3px;border:1px solid #ccc" rowspan="2">MARKETING</td>
					<td style="background:#f8f9fc;padding:3px;border:1px solid #ccc" rowspan="2">CUSTOMER</td>
					<td style="background:#f8f9fc;padding:3px;border:1px solid #ccc;position:sticky;left:0" rowspan="2">NAMA BARANG</td>';
					for ($i = 1; $i <= $hari; $i++) {
						($i < 10) ? $a = '0'.$i : $a = $i;
						// CEK LIBUR
						$tglLbr = $tahun.'-'.$bulan.'-'.$a;
						...
						$html2 .= '
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center">SALDO</td>
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center" rowspan="2">IN</td>
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center">RETUR</td>
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center" rowspan="2">OUT</td>
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center">RETUR</td>
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center">SALDO</td>
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center" rowspan="2">TONASE</td>
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center" rowspan="2">KETERANGAN</td>
						';
					}
				$html2 .= '</tr>';
				$html2 .= '<tr>';
					for ($i = 1; $i <= $hari; $i++) {
						...
						$html2 .= '
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center">AWAL</td>
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center">IN</td>
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center">OUT</td>
							<td style="background:'.$kk.';padding:3px;border:1px solid '.$kb.';text-align:center">AKHIR</td>
						';
					}
				$html2 .= '</tr>';
				...
				if($pelanggan->num_rows() != 0){
					foreach($pelanggan->result() as $p){
						...
						$html2 .= '<tr class="list-gd">
							<td style="background:#fff;padding:3px;border:1px solid #ccc">'.$p->nm_sales.'</td>
							<td style="background:#fff;padding:3px;border:1px solid #ccc">'.$p->nm_pelanggan.$attn.'</td>
							<td style="background:#fff;padding:3px;border:1px solid #ccc;position:sticky;left:0">'.$dv1.$p->nm_produk.$dv2.'</td>';

							for ($i = 1; $i <= $hari; $i++) {
								($i < 10) ? $a1 = '0'.$i : $a1 = $i;
								// CEK LIBUR
								$tglLbr = $tahun.'-'.$bulan.'-'.$a1;
								$libur = $this->db->query("SELECT*FROM libur WHERE tgl='$tglLbr'");
								$namaHari = date('l', strtotime($tglLbr));
								if($libur->num_rows() != 0){
									$kk = 'background:#fcc';
									...
									($namaHari == "Sunday") ? $kk = 'background:#fcc' : $kk = '';
									($namaHari == "Sunday") ? $kb = '#daa' : $kb = '#ccc';
								}

								$tglNow = strtotime($tglLbr) - strtotime(date('Y-m-d'));
								($tglNow > 0) ? $dsb = 'readonly' : $dsb = '';
								...
								$vSA = ($qq->row($a1.'_stok_awal') == NULL) ? '' : number_format($qq->row($a1.'_stok_awal'),0,',','.');
								$vIN = ($qq->row($a1.'_in') == NULL) ? '' : number_format($qq->row($a1.'_in'),0,',','.');
								$vINrtr = ($qq->row($a1.'_in_rtr') == NULL) ? '' : number_format($qq->row($a1.'_in_rtr'),0,',','.');
								$vOUT = ($qq->row($a1.'_out') == NULL) ? '' : number_format($qq->row($a1.'_out'),0,',','.');
	...
	function loadGC()
	{
		...
		$pilih_tgl22 = $_POST["pilih_tgl22"];
		$hari2 = date('d', strtotime($pilih_tgl22));
		$bulan2 = date('m', strtotime($pilih_tgl22));
		$tahun2 = date('Y', strtotime($pilih_tgl22));
		$w2 = 'AND '.$hari2.'_stok_akhir IS NOT NULL';
		...
		if($tgl_awal2 == ''){
			...
			$html .= 'PILIH TANGGAL DAHULU!';
		}else if($tgl_awal2 == $pilih_tgl22){
			$html .= 'PILIH TGL TIDAK BOLEH SAMA!';
		}else if($pilih_tgl22 < $tgl_awal2){
			$html .= 'PILIH TGL LEBIH KECIL DARI TGL STOK AWAL!';
		}else if($gudang2->num_rows() != 0){
			$html .= 'DATA STOK GUDANG SUDAH ADA!';
		}else{
			...
			if($pelanggan->num_rows() == 0){
				$html .= 'DATA STOK GUDANG KOSONG!';
			}else{
				$html .= '<form role="form" method="post" id="listForm">
					<input type="hidden" id="h_tgl_awal2" name="h_tgl_awal2" value="'.$tgl_awal2.'">
					<div style="padding:0;overflow:auto;white-space:nowrap">
						<table class="table table-bordered table-striped" style="margin:0;border:0">
							<tr>
								<th style="padding:6px" colspan="13">
									...
									&nbsp
									<button type="button" class="btn btn-xs btn-warning" style="padding:1px 5px;font-weight:bold;cursor:default">
										<span>[ KELUAR SURAT JALAN ]</span>
									</button>
							...
							foreach($pelanggan->result() as $p){
								...
								$html .= '<tr class="tr1 t'.$p->id_pelanggan.'" style="display:none">
								...
								foreach($gudang->result() as $g){
									...
									$cekPO = $this->db->query("SELECT*FROM trs_po p
									INNER JOIN trs_po_detail d ON p.no_po=d.no_po AND p.kode_po=d.kode_po
									WHERE p.status!='Close' AND p.status_kiriman='Open' AND p.id_pelanggan='$g->id_pelanggan' AND d.id_produk='$g->id_produk'
									GROUP BY p.status DESC,p.tgl_po,p.kode_po");
									($cekPO->num_rows() != 0) ? $infoPO = '<button type="button" class="btn btn-sm" style="padding:0" onclick="listPO('."'".$g->id_pelanggan."'".', '."'".$g->id_produk."'".', '."'LIST'".')"><i class="fas fa-info-circle" style="color:#0d6efd"></i></button> ' : $infoPO = '';

									// CEK JIKA ADA KIRIMAN
									$cekKirim = $this->db->query("SELECT*FROM m_rencana_kirim r WHERE r.rk_tgl='$pilih_tgl22' AND r.id_produk='$g->id_produk' GROUP BY r.id_produk");
									($cekKirim->num_rows() != 0) ? $bb = 'background:#ffa;border:1px solid #dd8;' : $bb = ''; //ffc107
									...
									$html .= '<tr class="tr1 t'.$g->id_pelanggan.'" style="vertical-align:top;display:none">
										...
										<td style="'.$bb.'padding:6px">
											<input type="number" id="stok_awal2_'.$g->id_produk.'" name="stok_awal2_'.$g->id_produk.'" value="'.$vSk.'" class="form-control" style="padding:2px 4px;text-align:right;font-weight:bold;color:#000" readonly>
										</td>
										<td style="'.$bb.'padding:6px">
											<input type="number" id="in2_'.$g->id_produk.'" name="in2_'.$g->id_produk.'" value="0" onkeyup="keyUpGD2('."'".$g->id_produk."'".')" class="form-control" placeholder="0" style="padding:2px 4px;text-align:right;font-weight:bold;color:#000" '.$roL.'>
										</td>
										<td style="'.$bb.'padding:6px">
											<input type="number" id="inrtr2_'.$g->id_produk.'" name="inrtr2_'.$g->id_produk.'" value="0" onkeyup="keyUpGD2('."'".$g->id_produk."'".')" class="form-control" placeholder="0" style="padding:2px 4px;text-align:right;font-weight:bold;color:#000" '.$roL.'>
										</td>
										<td style="'.$bb.'padding:6px">
											<input type="number" id="out2_'.$g->id_produk.'" name="out2_'.$g->id_produk.'" value="0" onkeyup="keyUpGD2('."'".$g->id_produk."'".')" class="form-control" placeholder="0" style="padding:2px 4px;text-align:right;font-weight:bold;color:#000" '.$roL.'>
										</td>
										<td style="'.$bb.'padding:6px">
											<input type="number" id="outrtr2_'.$g->id_produk.'" name="outrtr2_'.$g->id_produk.'" value="0" onkeyup="keyUpGD2('."'".$g->id_produk."'".')" class="form-control" placeholder="0" style="padding:2px 4px;text-align:right;font-weight:bold;color:#000" '.$roL.'>
										</td>
										<td style="'.$bb.'padding:6px">
											<input type="hidden" id="hstok_akhir2_'.$g->id_produk.'" name="hstok_akhir2_'.$g->id_produk.'" value="'.$vSkh.'">
											<input type="number" id="stok_akhir2_'.$g->id_produk.'" name="stok_akhir2_'.$g->id_produk.'" value="'.$vSk.'" class="form-control" placeholder="0" style="padding:2px 4px;text-align:right;font-weight:bold;color:#000" disabled>
										</td>
										<td style="'.$bb.'padding:6px">
											<input type="hidden" id="hTTON2_'.$g->id_produk.'" name="hTTON2_'.$g->id_produk.'" value="'.$g->berat_bersih.'">
											<input type="number" id="tton2_'.$g->id_produk.'" name="tton2_'.$g->id_produk.'" value="'.$vTon.'" class="form-control" placeholder="0" style="padding:2px 4px;text-align:right;font-weight:bold;color:#000" disabled>
										</td>
										<td style="'.$bb.'padding:6px">
											<input type="text" id="ket2_'.$g->id_produk.'" name="ket2_'.$g->id_produk.'" class="form-control" placeholder="KETERANGAN" autocomplete="off" style="padding:2px 4px;font-weight:bold;color:#000" oninput="this.value=this.value.toUpperCase()" '.$roL.'>
										</td>
									</tr>';
								}
							}
					...
					if(in_array($this->session->userdata('level'), ['Admin', 'Gudang'])){
						$html .= '<div class="card-body" style="padding:12px 0">
							<div class="card-body row" style="padding:6px 0">
								<div class="col-md-12">
									<input type="hidden" id="pilih_tgl2" name="pilih_tgl2" value="'.$pilih_tgl22.'">
		...
		$html .= '<table style="width:100%;margin-top:25px;color:#000;border-collapse:collapse;vertical-align:top;font-family:tahoma;font-size:12px">
			...
			<tbody>';
				...
				foreach($pelanggan->result() as $p){
					...
					WHERE g.id_pelanggan='$p->id_pelanggan' $wA
					ORDER BY p.kategori,p.nm_produk");
					$i = 0; $sumAwal = 0; $sumIN = 0; $sumINrtr = 0; $sumOUT = 0; $sumOUTrtr = 0; $sumAkhir = 0;
					foreach($gudang->result() as $g){
						$i++;
						($g->kategori == 'K_BOX') ? $kat = '' : $kat = '[SHEET] ';
						($g->kategori == 'K_BOX') ? $uk = $g->ukuran : $uk = $g->ukuran_sheet;
						...
						$vSa = ($qq->row($hari.'_stok_awal') == 0) ? 0 : number_format($qq->row($hari.'_stok_awal'),0,',','.');
						$vIn2 = ($qq->row($hari.'_in') == 0) ? 0 : number_format($qq->row($hari.'_in'),0,',','.');
						$vIn2Rtr = ($qq->row($hari.'_in_rtr') == 0) ? '' : ' ('.number_format($qq->row($hari.'_in_rtr'),0,',','.').')';
						$vOut = ($qq->row($hari.'_out') == 0) ? 0 : number_format($qq->row($hari.'_out'),0,',','.');
						$vOutRtr = ($qq->row($hari.'_out_rtr') == 0) ? '' : ' ('.number_format($qq->row($hari.'_out_rtr'),0,',','.').')';
						$vSk = ($qq->row($hari.'_stok_akhir') == 0) ? 0 : number_format($qq->row($hari.'_stok_akhir'),0,',','.');
						$vKet = $qq->row($hari.'_ket');
						$html .= '<tr style="vertical-align:top">
							<td style="border:1px solid #ccc;padding:6px;text-align:center">'.$i.'</td>
							...
							<td style="border:1px solid #ccc;padding:6px;text-align:right">'.$vIn2.$vIn2Rtr.'</td>
							<td style="border:1px solid #ccc;padding:6px;text-align:right">'.$vOut.$vOutRtr.'</td>
						...
						$sumINrtr += ($qq->row($hari.'_in_rtr') == 0) ? 0 : $qq->row($hari.'_in_rtr');
						$sumOUT += ($qq->row($hari.'_out') == 0) ? 0 : $qq->row($hari.'_out');
						$sumOUTrtr += ($qq->row($hari.'_out_rtr') == 0) ? 0 : $qq->row($hari.'_out_rtr');
						$sumAkhir += ($qq->row($hari.'_stok_akhir') == 0) ? 0 : $qq->row($hari.'_stok_akhir');
					}
					// ALL
					$allAwal += $sumAwal;
					$allIN += $sumIN + $sumINrtr;
					$allOUT += $sumOUT + $sumOUTrtr;
					$allAkhir += $sumAkhir;
				}
				// TOTAL
				$html .= '<tr style="background:#5eafde">
			...
			if($opsi == "no_po"){
				$html .= '<table style="margin-top:6px;border:1px solid #dee2e6">
					...
					foreach($getCustomer->result() as $gdi){
						...
						if($qty == 0){
							$this->db->query("UPDATE m_gudang SET gd_status='Close' WHERE id_gudang='$gdi->id_gudang'");
						}
						...
						if($qty > 0){
							$html .='<tr style="vertical-align:top">
								...
								<td style="border:1px solid #dee2e6;padding:6px;text-align:right">'.$gdi->gd_berat_box.'</td>
								<td style="border:1px solid #dee2e6;padding:6px">
									<input type="number" class="form-control" style="height:100%;width:70px;text-align:right;padding:4px 6px" id="inp-muat-'.$gdi->id_gudang.'" onkeyup="hitungSJTonase('."'".$gdi->id_gudang."'".')">
								</td>
								<td style="border:1px solid #dee2e6;padding:6px;text-align:right;font-weight:bold">
									<span class="hitung-tonase-'.$gdi->id_gudang.'">0</span>
									<input type="hidden" id="hidden-hitung-tonase-'.$gdi->id_gudang.'" value="">
									<input type="hidden" id="hidden-id-pelanggan-'.$gdi->id_gudang.'" value="'.$gdi->gd_id_pelanggan.'">
									<input type="hidden" id="hidden-id-produk-'.$gdi->id_gudang.'" value="'.$gdi->gd_id_produk.'">
									<input type="hidden" id="hidden-nm-pelanggan-'.$gdi->id_gudang.'" value="'.$gdi->nm_pelanggan.'">
									<input type="hidden" id="hidden-nm-produk-'.$gdi->id_gudang.'" value="'.$gdi->nm_produk.'">
									<input type="hidden" id="hidden-kategori-'.$gdi->id_gudang.'" value="'.$kategori2.'">
									<input type="hidden" id="hidden-kode-po-'.$gdi->id_gudang.'" value="'.$gdi->gd_kode_po.'">
									<input type="hidden" id="hidden-bb-'.$gdi->id_gudang.'" value="'.$gdi->gd_berat_box.'">
									<input type="hidden" id="hidden-qty-'.$gdi->id_gudang.'" value="'.$qty.'">
								</td>
		...
		if($cekRev->num_rows() != 0){
			$html .='<table>
				...
				foreach($cekRev->result() as $v){
					...
					$html .='<tr>
						<td style="background:#333;color:#fff;padding:6px" colspan="2">'.$txtTgl.$btnPengiriman.$txtUrut.'</td>
						<td style="background:#333;color:#fff;padding:6px" colspan="7"></td>
					</tr>';
					...
					foreach($getRev->result() as $r){
						...
						$html .='<tr>
							...
							<td style="border:1px solid #dee2e6;padding:6px;text-align:right">'.$r->rk_bb.'</td>
							<td style="border:1px solid #dee2e6;padding:6px;text-align:right;font-weight:bold">'.number_format($r->rk_tonase).'</td>
						</tr>';
					}
				}
			$html .='</table>';
	...
	function batalRev()
	{
		$result = $this->m_logistik->batalRev();
		echo json_encode($result);
	}
			...
			$html .='<table>
				...
				foreach($getUrut->result() as $urut){
					...
					$html .='<tr id="urut'.$urut->no_pl_urut.'">
						...
						<td style="background:#333;color:#fff;text-align:center;padding:6px">
					...
					foreach($getSJnPO->result() as $sjpo){
						...
						// CEK INV
						if($sjpo->no_pl_inv == 0 && $tglNow != $urut->tgl && in_array($this->session->userdata('level'), ['Admin', 'Admin2', 'User'])){
							($sjpo->kategori == 'SHEET') ? $bBp = 'btn-dark' : $bBp = 'btn-danger';
							$btnInv = '<button type="button" class="btn btn-xs '.$bBp.'" style="font-weight:bold" onclick="batalRev('."'".$sjpo->id."'".')">BATAL</button>&nbsp';
						}else{
							$btnInv = '';
						}
