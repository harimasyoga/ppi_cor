...
class Logistik extends CI_Controller
{
		...
		if($jenis == 'CUSTOMER'){
			...
			if($query->num_rows() == 0){
				...
				foreach($query->result() as $r){
					...
					INNER JOIN m_produk_lm i ON r.id_m_produk_lm=i.id_produk_lm
					WHERE l.tgl BETWEEN '$tgl1' AND '$tgl2' AND i.jenis_qty_lm='pack' AND r.id_pelanggan_lm='$r->id_perusahaan' AND l.attn_pl='$r->attn_pl'
					...
					INNER JOIN pl_laminasi l ON r.id_pl_lm=l.id AND r.rk_urut=l.no_pl_urut AND r.rk_no_po=l.no_po AND r.rk_tgl=l.tgl AND r.id_pelanggan_lm=l.id_perusahaan
					INNER JOIN m_produk_lm i ON r.id_m_produk_lm=i.id_produk_lm
					WHERE l.tgl BETWEEN '$tgl1' AND '$tgl2' AND i.jenis_qty_lm='ikat' AND r.id_pelanggan_lm='$r->id_perusahaan' AND l.attn_pl='$r->attn_pl'
					AND r.rk_status='Close' GROUP BY r.id_pelanggan_lm");
					...
					INNER JOIN pl_laminasi l ON r.id_pl_lm=l.id AND r.rk_urut=l.no_pl_urut AND r.rk_no_po=l.no_po AND r.rk_tgl=l.tgl AND r.id_pelanggan_lm=l.id_perusahaan
					INNER JOIN m_produk_lm i ON r.id_m_produk_lm=i.id_produk_lm
					WHERE l.tgl BETWEEN '$tgl1' AND '$tgl2' AND i.jenis_qty_lm='kg' AND r.id_pelanggan_lm='$r->id_perusahaan' AND l.attn_pl='$r->attn_pl'
					AND r.rk_status='Close' GROUP BY r.id_pelanggan_lm");
		...
		$htmlItem .='<table class="table table-bordered" style="margin:0">
			...
			if($disc->num_rows() > 0){
				foreach($disc->result() as $c){
					...
					$htmlDisc .='<tr>
						<td style="border:0;padding:6px;text-align:right;font-weight:bold" colspan="9">
							'.$ketDsc.$btnDsc.'
						</td>
				...
				$htmlDisc .='<tr>
					<td style="border:0;padding:6px;text-align:right;font-weight:bold" colspan="9">Total</td>
			...
			$htmlItem .='<tr>
				<td style="border:0;padding:6px;text-align:right;font-weight:bold" colspan="9">'.$txtTot.'</td>
	...
	function cetakInvoiceLaminasi()
	{
		...
		$html .= '<table cellspacing="0" style="font-size:11px;color:#000;border-collapse:collapse;vertical-align:top;width:100%;font-family:tahoma">
			...
			// PPI
			if($header->bank == 7 || $header->bank == 'IHI'){
				$html .='<tr>
		...
		if($no_surat == ''){
			...
			$htmlItem .='<table class="table table-bordered" style="margin:0">
				...
				foreach($isi->result() as $r){
					...
					$tonase = round($r->berat_bersih * $r->muat);
					$htmlItem .='<tr>
						<td style="padding:6px;text-align:center">'.$i.'</td>
						...
						<td style="padding:6px;text-align:center;text-align:right">'.number_format($r->muat,0,',','.').'</td>
	...
	function editInvoiceJasa()
	{
		...
		$htmlItem .='<table class="table table-bordered" style="margin:0">
			<tr style="background:#f8f9fc">
				...
				<th style="padding:6px 12px;border-bottom:1px solid #6c757d;text-align:center">BB</th>
				<th style="padding:6px 12px;border-bottom:1px solid #6c757d;text-align:center">TONASE</th>
				<th style="padding:6px;border-bottom:1px solid #6c757d;text-align:center">HARGA</th>
				...
				$htmlItem .='<tr>
					...
					<td style="padding:0;border:0">
						<table class="table" style="margin:0;border:0">
							...
							<tr><td style="border:0;padding:6px">'.$kualitas.'</td></tr>
						</table>
					</td>
					<td style="padding:6px;text-align:center">'.number_format($r->qty_muat,0,',','.').'</td>
					<td style="padding:6px;text-align:center">'.$r->berat_bersih.'</td>
					<td style="padding:6px;text-align:center;font-weight:bold">'.number_format($tonase,0,',','.').'</td>';
					...
					$htmlItem .='<td style="padding:6px;text-align:center">
						<input type="hidden" id="tonase-'.$r->id_dtl.'" value="'.$tonase.'">
