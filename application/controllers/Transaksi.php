...
class Transaksi extends CI_Controller
{
		...
		if($id_pt != ''){
			...
			$html .= '<table style="color:#000">
				...
				if($groupBy == ''){
					...
					INNER JOIN pl p ON t.id_pl=p.id WHERE p.no_po=po.no_po AND t.nm_ker=po.nm_ker
					...
					INNER JOIN pl p ON t.id_pl=p.id WHERE p.no_po=po.no_po AND t.nm_ker=po.nm_ker AND t.g_label=po.g_label AND t.width=po.width AND p.id_perusahaan=po.id_perusahaan) AS kirim_tonase,";
			...
			foreach($noPO->result() as $n){
				$html .= '<table style="color:#000;margin-bottom:12px">
					...
					foreach($list->result() as $r){
						...
						$i++;
						$round = ((rand(10,123) + $i) * rand(1,23)) - rand(1,23);
						$html .= '<tr'.$bgR.'>
							<td style="padding:6px;border:1px solid #888;text-align:center">'.$r->nm_ker.'</td>
							<td style="padding:6px;border:1px solid #888;text-align:center">'.$r->g_label.'</td>
							<td style="padding:6px;border:1px solid #888;text-align:center">
	...
	function editPOLaminasi()
	{
		...
		$html .='<table class="table table-bordered table-striped" style="margin:0 0 12px">
			...
			foreach($po_dtl->result() as $r){
				...
				// 	$isi1 = '<tr><td style="border:0;padding:6px">'.$r->ikat_x.'</td></tr>
				// 	<tr><td style="border:0;padding:6px">'.$r->pack_x.'</td></tr>
				// 	<tr><td style="border:0;padding:6px">'.$qty.' '.$ket.'</td></tr>';
				...
				// }
				if($r->jenis_qty_lm == 'pack'){
					($r->ikat_x == null) ? $ikat_x = '-' : $ikat_x = $r->ikat_x;
					($r->pack_x == null) ? $pack_x = '-' : $pack_x = $r->pack_x;
					$ket1 = '<tr><td style="border:0;padding:6px;font-weight:bold">IKAT</td></tr>
					<tr><td style="border:0;padding:6px;font-weight:bold">PACK</td></tr>';
					$t1 = '<tr><td style="border:0;padding:6px 3px;font-weight:bold">:</td></tr>
					<tr><td style="border:0;padding:6px 3px;font-weight:bold">:</td></tr>';
					$i1 = '<tr><td style="border:0;padding:6px">'.$ikat_x.'</td></tr>
					<tr><td style="border:0;padding:6px">'.$pack_x.'</td></tr>';
				}else{
					$ket1 = '';
					$t1 = '';
					$i1 = '';
				...
				$html .='<tr>
					...
					<td style="padding:0;border:0'.$bold.'">
						<table class="table" style="margin:0">
							...
							<tr><td style="border:0;padding:6px">'.$order_pori_lm.' '.$ket.'</td></tr>
	...
	function verifKecil()
	{
		$result = $this->m_transaksi->verifKecil();
		echo json_encode($result);
	}
	...
	function hpsKetPO()
	{
		$result = $this->m_transaksi->hpsKetPO();
		echo json_encode($result);
	}
	...
	function load_data()
	{
		...
		if ($jenis == "po") {
			...
			foreach ($query as $r) {
				...
				if($r->status_app1=='N'){
					if($actualDate > $expired || $actualDate == $expired){
						...
						$alasan1    = '';
						$ketAlasan1 .= '<div style="color:#dc3545;font-weight:bold">EXPIRED</div>';
						$exp1 .= 'expired';
					}else{
						$btn1       = 'btn-warning';
						$i1         = '<i class="fas fa-lock"></i>';
						$alasan1    = '';
						$ketAlasan1 .= '<div style="color:#dc3545;font-weight:bold">'.$waktu.'</div>';
					}
                ...
                if($r->status_app2=='N'){
					if($actualDate > $expired || $actualDate == $expired){
						...
						$alasan2    = '';
						$ketAlasan2 .= '<div style="color:#dc3545;font-weight:bold">EXPIRED</div>';
						$exp2 .= 'expired';
					}else{
						$btn2       = 'btn-warning';
						$i2         = '<i class="fas fa-lock"></i>';
						$alasan2    = '';
						$ketAlasan2 .= '<div style="color:#dc3545;font-weight:bold">'.$waktu.'</div>';
					}
                ...
                if($r->status_app4=='N'){
					if($actualDate > $expired || $actualDate == $expired){
						...
						$alasan4    = '';
						$ketAlasan4 .= '<div style="color:#dc3545;font-weight:bold">EXPIRED</div>';
						$exp4 .= 'expired';
					}else{
						$btn4       = 'btn-warning';
						$i4         = '<i class="fas fa-lock"></i>';
						$alasan4    = '';
						$ketAlasan4 .= '<div style="color:#dc3545;font-weight:bold">'.$waktu.'</div>';
					}
			...
			foreach ($query as $r) {
				...
				($r->input_po == 'N' && $r->owner_status == 'Y' && ($this->session->userdata('level') == 'Admin' || $this->session->userdata('level') == 'Admin2')) ? $btnUpload = '<button type="button" title="INPUT PO" class="btn btn-danger btn-sm" onclick=""><i class="fas fa-file-upload"></i></button>' : $btnUpload = '';
				if($this->session->userdata('level') == 'Admin' || $this->session->userdata('level') == 'Admin2'){
			...
			if($devSys->num_rows() != 0){
				$html .= '<table>';
					...
					foreach($devSys->result() as $r){
						...
						$sumQty += ($r->eta_t == 'REPLAN') ? 0 : $r->qty_plan;
					}
	...
	function get_edit()
	{
		...
		if ($jenis == "trs_po") {
			...
			if($this->session->userdata('level') == 'User'){
				$disK = ($header->status_app5 == 'Y') ? ' disabled' : ' onclick="verifKecil('."'in_kalkulator'".')"';
				$disD = ($header->status_app5 == 'Y') ? ' disabled' : ' onclick="verifKecil('."'in_ds'".')"';
				$disA = ($header->status_app5 == 'Y') ? ' disabled' : ' onclick="verifKecil('."'in_armada'".')"';
				$in_kalkulator = ($header->in_kalkulator == 0) ? 'value="0"' : 'value="1" checked';
				$in_ds = ($header->in_ds == 0) ? 'value="0"' : 'value="1" checked';
				$in_armada = ($header->in_armada == 0) ? 'value="0"' : 'value="1" checked';
				$verif .= '<table style="font-weight:bold">
					<tr>
						<td>Kalkulator</td>
						<td style="padding:0 6px">:</td>
						<td>
							<input type="checkbox" id="in_kalkulator" style="display:grid;grid-template-columns:1em auto;gap:0.5em" class="form-control" '.$in_kalkulator.$disK.'>
						</td>
					</tr>
					<tr>
						...
						<td style="padding:0 6px">:</td>
						<td>
							<input type="checkbox" id="in_ds" style="display:grid;grid-template-columns:1em auto;gap:0.5em" class="form-control" '.$in_ds.$disD.'>
						</td>
					</tr>
					<tr>
						<td>Armada</td>
						<td style="padding:0 6px">:</td>
						<td>
							<input type="checkbox" id="in_armada" style="display:grid;grid-template-columns:1em auto;gap:0.5em" class="form-control" '.$in_armada.$disA.'>
						</td>
					</tr>
				</table>';
			}
			...
			if($this->session->userdata('level') == 'Marketing'){
				$disH = ($header->status_app1 == 'Y') ? ' disabled' : ' onclick="verifKecil('."'mk_harga'".')"';
				$disQ = ($header->status_app1 == 'Y') ? ' disabled' : ' onclick="verifKecil('."'mk_qty'".')"';
				$disE = ($header->status_app1 == 'Y') ? ' disabled' : ' onclick="verifKecil('."'mk_eta1'".')"';
				$mk_harga = ($header->mk_harga == 0) ? 'value="0"' : 'value="1" checked';
				$mk_qty = ($header->mk_qty == 0) ? 'value="0"' : 'value="1" checked';
				$mk_eta1 = ($header->mk_eta1 == 0) ? 'value="0"' : 'value="1" checked';
				$verif .= '<table style="font-weight:bold">
					<tr>
						<td>Harga</td>
						<td style="padding:0 6px">:</td>
						<td>
							<input type="checkbox" id="mk_harga" style="display:grid;grid-template-columns:1em auto;gap:0.5em" class="form-control" '.$mk_harga.$disH.'>
						</td>
					</tr>
					<tr>
						<td>Qty</td>
						<td style="padding:0 6px">:</td>
						<td>
							<input type="checkbox" id="mk_qty" style="display:grid;grid-template-columns:1em auto;gap:0.5em" class="form-control" '.$mk_qty.$disQ.'>
						</td>
					</tr>
					<tr>
						<td>ETA KE 1</td>
						<td style="padding:0 6px">:</td>
						<td>
							<input type="checkbox" id="mk_eta1" style="display:grid;grid-template-columns:1em auto;gap:0.5em" class="form-control" '.$mk_eta1.$disE.'>
						</td>
					</tr>
				</table>';
			}
			if($this->session->userdata('level') == 'PPIC'){
				$disP = ($header->status_app2 == 'Y') ? ' disabled' : ' onclick="verifKecil('."'pp_plan'".')"';
				$pp_plan = ($header->pp_plan == 0) ? 'value="0"' : 'value="1" checked';
				$verif .= '<table style="font-weight:bold">
					<tr>
						<td>Plan Produksi</td>
						<td style="padding:0 6px">:</td>
						<td>
							<input type="checkbox" id="pp_plan" style="display:grid;grid-template-columns:1em auto;gap:0.5em" class="form-control" '.$pp_plan.$disP.'>
						</td>
					</tr>
				</table>';
			}
	...
	function detailSO()
	{
		...
		INNER JOIN trs_po po ON po.no_po=d.no_po AND po.kode_po=d.kode_po
		INNER JOIN m_produk p ON d.id_produk=p.id_produk
		WHERE d.no_po='$no_po' AND d.kode_po='$kode_po' AND d.id='$id'");
		...
		foreach($getSO->result() as $r){
			...
			WHERE s.id_produk='$r->id_produk' AND s.no_po='$r->no_po' AND s.kode_po='$r->kode_po' AND s.no_so='$r->no_so' AND s.add_user!='ppic'");
			...
			if($dataSO->num_rows() != 0){
						...
						<table class="table table-bordered" style="margin:0;border:0;width:100%">
							<thead>
								<tr>
									...
									<th style="width:10%;padding:6px 35px;'.$bHead.''.$bold.'" class="text-center">TON</th>
									<th style="width:10%;padding:6px 30px;'.$bHead.''.$bold.'" class="text-center">B. BAKU</th>
				...
				WHERE ps.id='$idPoSo' AND so.add_user!='ppic' GROUP BY so.no_po,so.kode_po,so.no_so,so.id_produk");
				...
				foreach($dataSO->result() as $so){
					$l++;
					// SYS
					$sys = $this->db->query("SELECT p.lock,s.* FROM trs_dev_sys s INNER JOIN m_pelanggan p ON s.id_pelanggan=p.id_pelanggan WHERE id_so='$so->id' ORDER BY s.id_dev");
					// ADD DS
					...
					if($sys->num_rows() != 0){
						...
						foreach($sys->result() as $s){
							...
							if($prov->num_rows() == 0){
								...
								$ll = $prov->row()->lama_kirim;
								$minggu = date('l', strtotime('+'.$ll.' day', strtotime($s->eta)));
								($minggu == 'Sunday') ? $ll2 = $prov->row()->lama_kirim + 1 : $ll2 = $prov->row()->lama_kirim;
								$eta = date('d/m/Y', strtotime('+'.$ll2.' day', strtotime($s->eta)));
							}

							// LOCK
							$lock3D = date('Y-m-d', strtotime('+'.$s->lock.' days', strtotime(date('Y-m-d'))));
							$selisihHariPilih = strtotime($lock3D) - strtotime($plh_eta);
							$hariPilih = floor($selisihHariPilih/60/60/24);
							$hPlus = floor((strtotime($s->eta) - strtotime($lock3D)) /60/60/24);
							$hPlus2 = floor((strtotime($s->eta) - strtotime(date('Y-m-d'))) /60/60/24);
							...
							if(in_array($lvl, ['Admin', 'Admin2', 'User', 'Marketing'])){
								...
								$eBsys = 'class="btn btn-secondary btn-xs" disabled';
								$hBsys = 'class="btn btn-secondary btn-xs" disabled';
							}
							...
							$html .= '<tr style="background:#f2f2f2">';
								...
								<td style="padding:6px;border:1px solid #999">
									<input type="text" id="se1ys'.$s->id_dev.'" class="form-control" style="font-weight:bold;text-align:center" value="'.$eta.'" disabled>
								</td>
								<td style="padding:6px;border:1px solid #999">
									<input type="number" id="sys_qty'.$s->id_dev.'" class="form-control" style="text-align:right;font-weight:bold" value="'.number_format($s->qty_plan,0,',','.').'" '.$diss.'>
								</td>
								<td style="padding:6px;border:1px solid #999">
									<textarea class="form-control" id="sys_ket'.$s->id_dev.'" oninput="this.value=this.value.toUpperCase()" rows="1" style="resize:none;font-weight:bold" '.$diss.' '.$plCh.'>'.$s->ket_sys.'</textarea>
								</td>
								<td style="padding:6px;border:1px solid #999;text-align:center">
									<input type="hidden" id="sys_qtylama'.$s->id_dev.'" value="'.$s->qty_plan.'">
									'.$ADSbtn.$btnRPlan.'
								</td>
							</tr>';
						}
	...
	{
		$result = $this->m_transaksi->addSysRePlan();
		echo json_encode($result);
	}
	...
	function btlRKtoSys()
	{
		$result = $this->m_transaksi->btlRKtoSys();
		echo json_encode($result);
	}
	...
	{
		$result = $this->m_transaksi->pilihDSRinc();
		echo json_encode($result);
	}
	...
	function tglMuatEtaDSys()
	{
		...
		if($prov->num_rows() == 0){
			...
			$ll = $prov->row()->lama_kirim;
			$eta = date('d/m/Y', strtotime('+'.$ll.' day', strtotime($eta_tiba)));
		}
	...
	function loadRealCalender()
	{
		...
		$html .= '<div>
			...
			$html .= '<div class="date-grid" style="border:1px solid #d9dadc;border-top:0">';
				...
				for ($i2 = 1; $i2 <= $hari; $i2++) {
					...
					$tonTon += ($count->num_rows() == 0) ? 0 : $berat;
				}
				// tambah kotak kosong akhir
				if($ak != 0) {
					for($i3 = 0; $i3 < $ak; $i3++){
		...
		// TOTAL
		if($tonTon != 0){
			$html .= '<div style="margin-top:5px;font-weight:bold">TONASE: '.number_format($tonTon,0,',','.').'</div>';
		}
	...
	function ccDevSys()
	{
		...
		if($opsi == 'kirim' || ($p_tgl != '' && $p_urut != '')){
			...
			$htmlSJ .='<table>
				...
				foreach($getUrut->result() as $urut){
					...
					$htmlPlan = '';
					if($qTimb->num_rows() == 0){
						$htmlPlan .= '<td style="border:1px solid #dee2e6;padding:6px;font-weight:bold;text-align:right;vertical-align:top" rowspan="'.$cSp.'">
							<button type="button" class="btn btn-sm btn-secondary" disabled><i class="fas fa-plus-circle"></i></button>
						</td>';
					}else if($qTimb->num_rows() > 0 && $d_rk->num_rows() == 0 && in_array($lvl, ['Admin', 'Admin2', 'User'])){
						$htmlPlan .= '<td style="border:1px solid #dee2e6;padding:6px;font-weight:bold;text-align:right;vertical-align:top" rowspan="'.$cSp.'">
							<button type="button" class="btn btn-sm btn-primary" style="font-weight:bold" onclick="pilihDSDS('."'".$tgl."'".', '."'".$urut->no_pl_urut."'".')"><i class="fas fa-plus-circle"></i></button>
						</td>';
					}else{
						$eta8 = $d_rk->row()->dev_tgl;
						$dev8 = $d_rk->row()->dev_urut;
						...
						INNER JOIN m_produk i ON d.id_produk=i.id_produk
						WHERE d.eta='$eta8' AND d.urut='$dev8'
						GROUP BY d.id_dev,d.id_pelanggan,p.kode_po,d.id_produk
						ORDER BY c.nm_pelanggan,p.kode_po,i.nm_produk");
						if($sys->num_rows() != 0){
							$htmlPlan .= '<td style="border:1px solid #dee2e6;padding:0;vertical-align:top" rowspan="'.$cSp.'">
								<table style="width:100%">
									<tr>
										<td style="background:#ddf;padding:6px;font-weight:bold;color:#ddf" colspan="6">'.$d_rk->row()->dev_urut.'</td>
									</tr>
									<tr>
										<td style="background:#dee2e6;text-align:center;font-weight:bold;padding:6px;border:1px solid #bbb">TANGGAL</td>
										<td style="background:#dee2e6;font-weight:bold;padding:6px;border:1px solid #bbb">CUSTOMER</td>
										<td style="background:#dee2e6;font-weight:bold;padding:6px;border:1px solid #bbb">NO. PO</td>
										<td style="background:#dee2e6;font-weight:bold;padding:6px;border:1px solid #bbb">ITEM</td>
										<td style="background:#dee2e6;text-align:center;font-weight:bold;padding:6px;border:1px solid #bbb">QTY</td>
										<td style="background:#dee2e6;text-align:center;font-weight:bold;padding:6px;border:1px solid #bbb">BERAT</td>
									</tr>';
									$xBB = 0; $xTot = 0; $x0 = 0;
									foreach($sys->result() as $x){
										$x0++;
										($x->attn == '-') ? $attn = '' : $attn = '<div>'.$x->attn.'</div>';
										(strlen($x->nm_produk) >= 35) ? $dv1 = '<div style="width:300px;white-space:normal">' : $dv1 = '';
										(strlen($x->nm_produk) >= 35) ? $dv2 = '</div>' : $dv2 = '';
										$htmlPlan .= '<tr style="vertical-align:top">
											<td style="padding:6px;border:1px solid #dee2e6">'.substr($this->m_fungsi->getHariIni($x->eta),0,3).', '.$this->m_fungsi->tglIndSkt($x->eta).'</td>
											<td style="padding:6px;border:1px solid #dee2e6">'.$x->nm_pelanggan.$attn.'</td>
											<td style="padding:6px;border:1px solid #dee2e6">'.$x->kode_po.'</td>
											<td style="padding:6px;border:1px solid #dee2e6">'.$dv1.$x->nm_produk.$dv2.'</td>
											<td style="padding:6px;border:1px solid #dee2e6;text-align:right">'.number_format($x->qty_plan, 0, ',', '.').'</td>
											<td style="padding:6px;border:1px solid #dee2e6;text-align:right">'.number_format($x->berat, 0, ',', '.').'</td>
										</tr>';
										$xBB += $x->qty_plan;
										$xTot += $x->berat;
									}
									...
									if($sys->num_rows() > 1){
										$htmlPlan .= '<tr>
											<td style="background:#dee2e6;padding:6px;border:1px solid #bbb" colspan="4"></td>
											<td style="background:#dee2e6;padding:6px;border:1px solid #bbb;font-weight:bold;text-align:right">'.number_format($xBB, 0, ',', '.').'</td>
											<td style="background:#dee2e6;padding:6px;border:1px solid #bbb;font-weight:bold;text-align:right">'.number_format($xTot, 0, ',', '.').'</td>
										</tr>';
									}
								$htmlPlan .= '</table>
							</td>';
		...
		if($opsi == 'jadwal'){
			// CEK PENGIRIMAN
			$aRK = $this->db->query("SELECT*FROM m_rencana_kirim WHERE (rk_tgl='$tgl' OR dev_tgl='$tgl') AND dev_urut IS NOT NULL GROUP BY rk_tgl");
			...
			$html .= '<table>
				...
				foreach($urut->result() as $u){
					...
					INNER JOIN m_pelanggan p ON r.id_pelanggan=p.id_pelanggan
					INNER JOIN m_produk i ON r.id_produk=i.id_produk
					WHERE r.dev_tgl='$u->eta' AND r.dev_urut='$u->urut' AND r.dev_id IS NULL
					...
					($cekKLB->num_rows() == 0 && $u->urut != 0) ? $txtKLB = '' : $txtKLB = $cekKLB->row()->kalibrasi;
					...
					$sys = $this->db->query("SELECT c.nm_pelanggan,c.attn,c.prov,c.kab,c.lock,p.kode_po,p.status AS sts,DATEDIFF(d.eta, CURDATE()) AS exp_dd,i.*,d.* FROM trs_dev_sys d
					INNER JOIN m_pelanggan c ON d.id_pelanggan=c.id_pelanggan
					INNER JOIN trs_po_detail p ON d.id_po_header=p.id
					INNER JOIN m_produk i ON d.id_produk=i.id_produk
					...
					foreach($sys->result() as $r){
						$i++;
						($r->attn == '-') ? $attn = '' : $attn = '<div>'.$r->attn.'</div>';
						($r->kategori == "K_BOX") ? $kategori = '' : $kategori = '[SHEET] ';
						if($cekKLB->num_rows() != 0 || $fileKLB->num_rows() != 0){
						...
						// STOK
						$wCS = date('d').'_stok_akhir';
						$cekSTOK = $this->db->query("SELECT $wCS AS stok_akhir,updated_at FROM m_gudang_v2 WHERE bulan='$bulan' AND tahun='$tahun' AND id_pelanggan='$r->id_pelanggan' AND id_produk='$r->id_produk' AND $wCS IS NOT NULL ORDER BY id DESC LIMIT 1");
						...
						if($tglNow <= 0){
							if($cekSTOK->num_rows() == 0){
								...
								$xS1 = $xH1.'_stok_akhir';
								$x1 = $this->db->query("SELECT $xS1 AS stok_akhir,updated_at FROM m_gudang_v2 WHERE bulan='$xB1' AND tahun='$xT1' AND id_pelanggan='$r->id_pelanggan' AND id_produk='$r->id_produk' AND $xS1 IS NOT NULL ORDER BY id DESC LIMIT 1");
								if($x1->num_rows() == 0 || ($x1->num_rows() != 0 && $x1->row()->stok_akhir == null)){
									...
									$xS2 = $xH2.'_stok_akhir';
									$x2 = $this->db->query("SELECT $xS2 AS stok_akhir,updated_at FROM m_gudang_v2 WHERE bulan='$xB2' AND tahun='$xT2' AND id_pelanggan='$r->id_pelanggan' AND id_produk='$r->id_produk' AND $xS2 IS NOT NULL ORDER BY id DESC LIMIT 1");
									if($x2->num_rows() == 0 || ($x2->num_rows() != 0 && $x2->row()->stok_akhir == null)){
										...
										$xS3 = $xH3.'_stok_akhir';
										$x3 = $this->db->query("SELECT $xS3 AS stok_akhir,updated_at FROM m_gudang_v2 WHERE bulan='$xB3' AND tahun='$xT3' AND id_pelanggan='$r->id_pelanggan' AND id_produk='$r->id_produk' AND $xS3 IS NOT NULL ORDER BY id DESC LIMIT 1");
										if($x3->num_rows() == 0 || ($x3->num_rows() != 0 && $x3->row()->stok_akhir == null)){
											...
											$xS4 = $xH4.'_stok_akhir';
											$x4 = $this->db->query("SELECT $xS4 AS stok_akhir,updated_at FROM m_gudang_v2 WHERE bulan='$xB4' AND tahun='$xT4' AND id_pelanggan='$r->id_pelanggan' AND id_produk='$r->id_produk' AND $xS4 IS NOT NULL ORDER BY id DESC LIMIT 1");
											if($x4->num_rows() == 0 || ($x4->num_rows() != 0 && $x4->row()->stok_akhir == null)){
												...
												$xS5 = $xH5.'_stok_akhir';
												$x5 = $this->db->query("SELECT $xS5 AS stok_akhir,updated_at FROM m_gudang_v2 WHERE bulan='$xB5' AND tahun='$xT5' AND id_pelanggan='$r->id_pelanggan' AND id_produk='$r->id_produk' AND $xS5 IS NOT NULL ORDER BY id DESC LIMIT 1");
												if($x5->num_rows() == 0 || ($x5->num_rows() != 0 && $x5->row()->stok_akhir == null)){
													...
													$xS6 = $xH6.'_stok_akhir';
													$x6 = $this->db->query("SELECT $xS6 AS stok_akhir,updated_at FROM m_gudang_v2 WHERE bulan='$xB6' AND tahun='$xT6' AND id_pelanggan='$r->id_pelanggan' AND id_produk='$r->id_produk' AND $xS6 IS NOT NULL ORDER BY id DESC LIMIT 1");
													if($x6->num_rows() == 0 || ($x6->num_rows() != 0 && $x6->row()->stok_akhir == null)){
														...
														$xS7 = $xH7.'_stok_akhir';
														$x7 = $this->db->query("SELECT $xS7 AS stok_akhir,updated_at FROM m_gudang_v2 WHERE bulan='$xB7' AND tahun='$xT7' AND id_pelanggan='$r->id_pelanggan' AND id_produk='$r->id_produk' AND $xS7 IS NOT NULL ORDER BY id DESC LIMIT 1");
														if($x7->num_rows() == 0 || ($x7->num_rows() != 0 && $x7->row()->stok_akhir == null)){
															...
															$txtInpSTOK = date('Y-m-d', strtotime('-7 days', strtotime(date('Y-m-d')))); //$x7->row()->updated_at
														}
													}else{
														$txtSTOK = ($x6->row()->stok_akhir == 0) ? '-' : number_format($x6->row()->stok_akhir, 0, ',', '.'); $cntSTOK = $x6->row()->stok_akhir;
														$txtInpSTOK = date('Y-m-d', strtotime('-6 days', strtotime(date('Y-m-d')))); //$x6->row()->updated_at
													}
												}else{
													$txtSTOK = ($x5->row()->stok_akhir == 0) ? '-' : number_format($x5->row()->stok_akhir, 0, ',', '.'); $cntSTOK = $x5->row()->stok_akhir;
													$txtInpSTOK = date('Y-m-d', strtotime('-5 days', strtotime(date('Y-m-d')))); //$x5->row()->updated_at
												}
											}else{
												$txtSTOK = ($x4->row()->stok_akhir == 0) ? '-' : number_format($x4->row()->stok_akhir, 0, ',', '.'); $cntSTOK = $x4->row()->stok_akhir;
												$txtInpSTOK = date('Y-m-d', strtotime('-4 days', strtotime(date('Y-m-d')))); //$x4->row()->updated_at
											}
										}else{
											$txtSTOK = ($x3->row()->stok_akhir == 0) ? '-' : number_format($x3->row()->stok_akhir, 0, ',', '.'); $cntSTOK = $x3->row()->stok_akhir;
											$txtInpSTOK = date('Y-m-d', strtotime('-3 days', strtotime(date('Y-m-d')))); //$x3->row()->updated_at
										}
									}else{
										$txtSTOK = ($x2->row()->stok_akhir == 0) ? '-' : number_format($x2->row()->stok_akhir, 0, ',', '.'); $cntSTOK = $x2->row()->stok_akhir;
										$txtInpSTOK = date('Y-m-d', strtotime('-2 days', strtotime(date('Y-m-d')))); //$x2->row()->updated_at
									}
								}else{
									$txtSTOK = ($x1->row()->stok_akhir == 0) ? '-' : number_format($x1->row()->stok_akhir, 0, ',', '.'); $cntSTOK = $x1->row()->stok_akhir;
									$txtInpSTOK = date('Y-m-d', strtotime('-1 days', strtotime(date('Y-m-d')))); //$x1->row()->updated_at
								}
							}else{
						...
						if($tglNow <= 0){
							...
							foreach($sys2->result() as $ky => $v){
								...
								if($ky == 0) {
									...
									$txtOSrt2 = '';
									$txtSTOKrt2 = ' <span style="font-style:italic">('.number_format($STOKrt2,0,",",".").')</span>';
								}
								// tampil data
								if($v->id_dev == $r->id_dev){
									$txtSTOKrt3 .= $txtSTOKrt2;
						...
						$html .= '<tr style="vertical-align:top">
							...
							if($rk->num_rows() != 0){
								foreach($rk->result() as $k){
									($r->akses_hapus == 1) ? $uDel = '<button class="btn btn-xs btn-danger" onclick="btlRKtoSys('."'".$k->id_rk."'".', '."'".$tgl."'".', '."'".$u->urut."'".')"><i class="fas fa-trash"></i></button>' : $uDel = '';
									...
									$html .= '
										...
										<td style="padding:6px;border:1px solid #dee2e6'.$bdb.';font-weight:bold;text-align:right">'.$txtTimb.'</td>
					...
					if($rkNull->num_rows() != 0){
						foreach($rkNull->result() as $n){
							...
							$html .= '<tr style="vertical-align:top">
								<td style="border:1px solid #ced2d6;padding:6px;text-align:center">
								...
								<td style="background:#fdd;border:1px solid #dbb;padding:6px;font-weight:bold;text-align:right">'.number_format($bBerat2, 0, ',', '.').'</td>
								<td style="background:#fdd;border:1px solid #dbb;padding:6px"></td>
					...
					// TOTAL
					if($sys->num_rows() != 1 || (($sys->num_rows() == 1 || $sys->num_rows() > 1) && ($rkNull->num_rows() == 1 || $rkNull->num_rows() > 1))){
						$html .= '<tr style="background:#dee2e6">
							...
							$html .= '<td style="padding:6px;border:1px solid #bbb;font-weight:bold">';
								...
								if($u->urut != 0 && ($u->id_ex == null || $u->id_ex != null)){
									if($fileKLB->num_rows() == 0 && in_array($lvl, ['Admin', 'Admin2', 'User'])){
										$oKb3 = 'id="k-'.$u->urut.'" onchange="addKLB('."'".$u->urut."'".')"'; $dkb3 = '';
									}else{
										$oKb3 = ''; $dkb3 = 'disabled';
									}
									$html .= '<input type="number" class="form-control" '.$oKb3.' style="height:100%;width:55px;text-align:center;font-weight:bold;padding:2px 4px" value="'.$txtKLB.'" autocomplete="off" placeholder="0" '.$dkb3.'>';
								}else{
									$html .= '<input type="number" class="form-control" style="height:100%;width:55px;text-align:center;font-weight:bold;padding:2px 4px" placeholder="0" disabled>';
								}
							$html .= '</td>';

							// TIMBANGAN LEBIH DARI 1 ITEM
							if($cekRK->num_rows() != 0 || (($sys->num_rows() == 1 || $sys->num_rows() > 1) && ($rkNull->num_rows() == 1 || $rkNull->num_rows() > 1))){
								if($u->timb_tgl != null && $u->timb_urut != null){
									$qTimb = $this->db->query("SELECT*FROM m_jembatan_timbang WHERE urut_t='$u->timb_urut' AND tgl_t='$u->timb_tgl' GROUP BY urut_t,tgl_t");
									if($qTimb->num_rows() != 0){
										$txtTimb = number_format($qTimb->row()->berat_bersih,0,',','.');
	...
	function planDSS()
	{
		...
		($sys->kategori == "K_BOX") ? $kategori = '' : $kategori = '[SHEET] ';
		(strlen($sys->nm_produk) >= 35) ? $dv1 = '<div style="width:320px;white-space:normal">' : $dv1 = '';
		(strlen($sys->nm_produk) >= 35) ? $dv2 = '</div>' : $dv2 = '';
